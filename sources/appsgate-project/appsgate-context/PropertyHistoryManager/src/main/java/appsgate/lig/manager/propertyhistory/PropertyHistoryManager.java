...
public class PropertyHistoryManager implements PropertyManager, DynamicManager {
	...
	private final Logger logger = LoggerFactory.getLogger(PropertyHistoryManager.class);
	...
	private Properties addDefaultProperties(Properties prop_model) {
		...
		if (prop_model.get(DBConfig.DBNAME_KEY) == null)
			prop_model.put(DBConfig.DBNAME_KEY,
					DBNAME_DEFAULT.concat(String.valueOf(dbNameCounter++)));

		logger.debug(" -> loaded DB Name : " + prop_model.get(DBConfig.DBNAME_KEY));
