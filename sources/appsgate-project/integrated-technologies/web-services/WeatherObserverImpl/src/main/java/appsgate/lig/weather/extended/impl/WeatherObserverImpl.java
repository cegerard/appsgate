...
public class WeatherObserverImpl extends AbstractObjectSpec implements
	...
	public WeatherObserverImpl() {
		lock = new Object();
		super.appsgateUserType = "103"; // 103 stands for weather forecast
										// service
		super.appsgateDeviceStatus = "2"; // 2 means device paired (for a
											// device, not
		...
		if (currentLocation != null && currentLocation.length() > 0) {
			if (weatherService == null) {
				throw new WeatherForecastException(
						"No weather service available, aborting");
			}
			weatherService.addLocation(currentLocation);
		} else {
			throw new WeatherForecastException(
					"Trying to create a WeatherObserver with an empty or null location");
		}
	...
	private NotificationMsg fireDaylightNotificationMsg(boolean isDayLight) {
		if (isDayLight) {
			return fireWeatherNotificationMsg("daylightEvent", "sunset",
					"sunrise");
		} else {
			return fireWeatherNotificationMsg("daylightEvent", "sunrise",
					"sunset");
		}
	}

	/**
	 * Check associated Simplified code and if they are different fire the
	 * NotificationMsg
	 * 
	 * @param varName
	 * @param oldCode
	 ...
	 * @return
	 */
	private NotificationMsg fireWeatherCodeNotificationMsgIfNew(String varName,
			int oldCode, int newCode) {
		if (SimplifiedWeatherCodesHelper.getSimplified(oldCode) != SimplifiedWeatherCodesHelper
				.getSimplified(newCode)) {
			return fireWeatherNotificationMsg(varName,
					String.valueOf(SimplifiedWeatherCodesHelper
							.getSimplified(oldCode)),
					String.valueOf(SimplifiedWeatherCodesHelper
							.getSimplified(newCode)));
		}
		return null;
	...
	private NotificationMsg fireWeatherTempNotificationMsgIfNew(String varName,
	...
	public boolean isCurrentlyDaylight() throws WeatherForecastException {
		if (clock == null) {
			throw new WeatherForecastException(
					"No clock service bound, cannot determine daylight");
		}
		...
		if (sunrise == null) {
			throw new WeatherForecastException(
					"No sunrise date inforation, cannot determine daylight");
		}
		if (sunset == null) {
			throw new WeatherForecastException(
					"No sunset date inforation, cannot determine daylight");
		}		
	...
	@Override
	public boolean isCurrentWeatherCode(int simpleWeatherCode)
			throws WeatherForecastException {
		refresh();
		return (SimplifiedWeatherCodesHelper.getSimplified(currentWeather
				.getWeatherCode()) == simpleWeatherCode);
	...
	@Override
	public boolean isForecastWeatherCode(int dayForecast, int simpleWeatherCode)
			throws WeatherForecastException {
		return (getForecastWeatherCode(dayForecast) == simpleWeatherCode);
	}

	private void testDayForecast(int dayForecast)
			throws WeatherForecastException {
		if (dayForecast < 0 || forecasts == null
				|| dayForecast >= forecasts.size()) {
			throw new WeatherForecastException("dayForecast not available : "
					+ dayForecast);
		}
	}
	...
	public String getForecastWeatherString(int dayForecast)
		...
		return SimplifiedWeatherCodesHelper
				.getDescription(SimplifiedWeatherCodesHelper
						.getSimplified(getForecastWeatherCode(dayForecast)));
	}
	...
	public void refresh() throws WeatherForecastException {
		synchronized (lock) {
			...
			if (clock == null) {
				throw new WeatherForecastException(
						"No clock service bound, cannot refresh");
			}

			if (weatherService == null) {
				throw new WeatherForecastException(
						"No weather service bound, cannot refresh");
			}

			if (lastFetch < 0
					|| System.currentTimeMillis() - lastFetch > 10000
					|| lastPublicationdate == null
					|| !lastPublicationdate.equals(weatherService
							.getPublicationDate(currentLocation))) {
				logger.trace("refreshing weather data for " + currentLocation);

				checkAndUpdateCurrentWeather(weatherService
						.getCurrentWeather(currentLocation));
				checkAndUpdateForecasts(weatherService
			...
			if (currentWeather == null) {
				throw new WeatherForecastException(
						"No weather data retrieved, cannot refresh");
			}

			logger.trace("refreshing alarms for " + currentLocation);
			alarmSunrise = refreshAlarm(alarmSunrise, currentWeather.getSunrise(), sunrise==null?null:sunrise.getTime());
			sunrise = Calendar.getInstance();
			sunrise.setTime(currentWeather.getSunrise());
			alarmSunset = refreshAlarm(alarmSunset, currentWeather.getSunset(), sunset==null?null:sunset.getTime());
			sunset = Calendar.getInstance();
			sunset.setTime(currentWeather.getSunset());
			

			if (currentWoeid == null) { // should not change, so we get it only
										// once
	...
	private int refreshAlarm(int alarmId, Date newAlarmTime, Date previousAlarmTime)
			throws WeatherForecastException{
		logger.trace("refreshAlarm(int alarmId : " + alarmId
				+ ", Date newAlarmTime : "+ newAlarmTime
				+ ", Date previousAlarmTime : "+ previousAlarmTime + ")");
		if(newAlarmTime == null) {
			throw new WeatherForecastException("refreshAlarm(...), no new alarm to refresh, aborting");
		}
		
		Calendar cal = Calendar.getInstance();
		cal.setTime(newAlarmTime);
		
		if(!newAlarmTime.equals(previousAlarmTime)) {
			logger.trace("refreshAlarm(...), alarm time is different, we have to register again");
			// The alarm time is new we have to register it again
			if (alarmId >= 0) {
				logger.trace("refreshAlarm(...), urnregistering "+alarmId);
				clock.unregisterAlarm(alarmId);
			}
			return clock.registerPeriodicAlarm(cal, 24*60*60*1000, this);
			
		} else {
			// The alarm time is not new, checking if an event is registered
			logger.trace("refreshAlarm(...), alarm time is the same");
			if (alarmId < 0) {
				logger.trace("refreshAlarm(...), registering "+alarmId);
				return clock.registerPeriodicAlarm(cal, 24*60*60*1000, this);
			} else {
				logger.trace("refreshAlarm(...), alarm is left unchanged "+alarmId);
				return alarmId; // left unchanged
			}
		}
	}
	...
	private void checkAndUpdateCurrentWeather(CurrentWeather newWeather) {
		if (newWeather != null) {
			fireWeatherCodeNotificationMsgIfNew(
					CurrentWeather.CODE,
					(currentWeather == null ? -1 : currentWeather
							.getWeatherCode()), newWeather.getWeatherCode());

			fireWeatherTempNotificationMsgIfNew(
					CurrentWeather.TEMPERATURE,
					(currentWeather == null ? -999 : currentWeather
							.getTemperature()), newWeather.getTemperature());
	...
	private void checkAndUpdateForecasts(List<DayForecast> newForecasts) {
		if (newForecasts != null && newForecasts.size() > 0) {
			...
			for (DayForecast newForecast : newForecasts) {
				...
				fireWeatherCodeNotificationMsgIfNew(DayForecast.FORECAST
						+ CurrentWeather.CODE + i, (oldForecast == null ? -1
						: oldForecast.getCode()), newForecast.getCode());

				fireWeatherTempNotificationMsgIfNew(DayForecast.FORECAST
						+ DayForecast.MIN + i, (oldForecast == null ? -1
						: oldForecast.getMin()), newForecast.getMin());

				fireWeatherTempNotificationMsgIfNew(DayForecast.FORECAST
						+ DayForecast.MAX + i, (oldForecast == null ? -1
						: oldForecast.getMax()), newForecast.getMax());
				i++;
			}
	...
	public void alarmEventFired(int alarmEventId) {
		if (alarmEventId == alarmSunrise) {
			logger.debug("Alarm (clock) event : sunrise, eventId = "
					+ alarmEventId);
			fireDaylightNotificationMsg(true);
		} else if (alarmEventId == alarmSunset) {
			logger.debug("Alarm (clock) event : sunset, eventId = "
					+ alarmEventId);
			fireDaylightNotificationMsg(false);
		} else {
			logger.warn("Unknown clock event (not sunrise nor sunset) : "
					+ alarmEventId);
		}
		try {
			refresh();
		} catch (WeatherForecastException exc) {
			logger.warn("Exception occured following alarm event "
					+ exc.getStackTrace());
		}
	}
	...
	public String getPresentationURL() {
		logger.trace("getPresentationURL(), returning "
