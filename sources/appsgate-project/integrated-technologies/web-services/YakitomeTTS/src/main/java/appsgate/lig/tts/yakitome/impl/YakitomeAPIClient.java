 ...
 * 
 * @see https://www.yakitome.com/documentation/tts_api It should not handle the
 *      lifecycle of audio files (should be used as stateless)
 * @author thibaud
 * 
 */
public class YakitomeAPIClient implements YakitomeAPI {
	...
	/**
	 * will only search for the tts generation and audio file for a limited
	 * times
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see appsgate.lig.tts.yakitome.YakitomeAPI#configure(java.lang.String,
		...
		try {
			...
			if (availableVoices != null) {
				...
				while (it.hasNext()) {
					...
					for (int i = 0; arrayLang != null && i < arrayLang.length(); i++) {
	 ...
	 */
	@Override
	public JSONObject textToSpeech(String text, String voice, int speed)
			throws ServiceException {
		Map<String, String> urlParameters = new LinkedHashMap<String, String>();
		if (!checkVoice(voice)) {
			throw new ServiceException(
					"Voice does not exist cannot generate Text To Speech"
							+ voice);
		}

		urlParameters.put(API_KEY_PARAM, api_key_value);
		urlParameters.put(PARAM_SEPARATOR + VOICE_PARAM, voice);
		urlParameters.put(PARAM_SEPARATOR + SPEED_PARAM,
				String.valueOf(checkAndgetSpeed(speed)));
		urlParameters.put(PARAM_SEPARATOR + TEXT_PARAM, text);

		String result = HttpUtils.sendHttpsPost(
				YAKITOME_JSON_URL + TTS_SERVICE, initHeaders(), urlParameters,
				null);
	...
	public int checkAndgetSpeed(int speed) {
		...
		if (speed == 0) {
			logger.trace("checkAndgetSpeed(...), speed is 0, using default value : "
					+ DEFAULT_SPEED);
			return DEFAULT_SPEED;
		} else if (speed > MAX_SPEED) {
			logger.trace("checkAndgetSpeed(...), speed is too high using max value: "
					+ MAX_SPEED);
			return MAX_SPEED;
		} else if (speed < MIN_SPEED) {
			logger.trace("checkAndgetSpeed(...), speed is too low using min value: "
					+ MIN_SPEED);
			return MIN_SPEED;
		} else {
			return speed;
	 ...
	 * 
	 * @see
	 * appsgate.lig.tts.yakitome.YakitomeAPI#getSpeechTextStatus(java.lang.String
	 * )
	 */
	@Override
	public JSONObject getSpeechTextStatus(int book_id) {
		...
		Map<String, String> urlParameters = new LinkedHashMap<String, String>();
		urlParameters.put(API_KEY_PARAM, api_key_value);
		urlParameters.put(PARAM_SEPARATOR + BOOK_ID_PARAM,
		...
		try {
			...
			logger.trace("getSpeechTextStatus(...), service exception occured :"
					+ exc.getMessage());
		}
	 ...
	 * 
	 * @see
	 * appsgate.lig.tts.yakitome.YakitomeAPI#getAudioFileURL(java.lang.String)
	 */
	@Override
	public JSONObject getAudioFileURL(int book_id) throws ServiceException {
		Map<String, String> urlParameters = new LinkedHashMap<String, String>();
		urlParameters.put(API_KEY_PARAM, api_key_value);
		urlParameters.put(PARAM_SEPARATOR + BOOK_ID_PARAM,
				String.valueOf(book_id));
	 ...
	 * 
	 * @see
	 * appsgate.lig.tts.yakitome.YakitomeAPI#deleteSpeechText(java.lang.String)
	 */
	@Override
	public JSONObject deleteSpeechText(int book_id) throws ServiceException {
		Map<String, String> urlParameters = new LinkedHashMap<String, String>();
		urlParameters.put(API_KEY_PARAM, api_key_value);
		urlParameters.put(PARAM_SEPARATOR + BOOK_ID_PARAM,
				String.valueOf(book_id));
	...
	public JSONObject waitForTTS(int book_id) {
		...
		while (testCounter < MAX_COUNTER && !found) {
			try {
				...
				logger.trace("waitForTTS(), sleep interrupted : "
						+ e.getMessage());
			}
			...
			try {
				...
				logger.trace("waitForTTS(), Service Exception : "
						+ e.getMessage());
				response = new JSONObject();
			}
			if (!response.has(YakitomeAPI.STATUS_KEY)
					|| YakitomeAPI.STATUS_RUNNING_VALUE.equals(response
							.getString(YakitomeAPI.STATUS_KEY))) {
				testCounter++;
				logger.trace("waitForTTS(), still not generated, research counter : "
						+ testCounter);

			} else if (YakitomeAPIClient.STATUS_DONE_VALUE.equals(response
					.getString(YakitomeAPI.STATUS_KEY))) {
				found = true;
			}
		}
		...
		if (!found) {
			logger.warn("waitForTTS(), Text To Speech not generated "
					+ "for {} after {}secs, giving up the research", book_id,
					SLEEP_PERIOD * MAX_COUNTER / 1000);
			return null;
		}

		logger.trace("waitForTTS(), step two generation of audio file for id : "
				+ book_id);
		...
		while (testCounter < MAX_COUNTER && !found) {
			...
			if (!response.has(YakitomeAPI.STATUS_KEY)
					...
					|| response.getJSONArray(YakitomeAPIClient.AUDIOS_KEY)
							.length() < 1) {
				testCounter++;
				logger.trace("waitForTTS(), still no audio file, research counter : "
						+ testCounter);
		...
		if (!found) {
			logger.warn(
					"waitForTTS(), Audio File not found {} after {}secs, giving up the research",
					book_id, SLEEP_PERIOD * MAX_COUNTER / 1000);
			return null;
		}
