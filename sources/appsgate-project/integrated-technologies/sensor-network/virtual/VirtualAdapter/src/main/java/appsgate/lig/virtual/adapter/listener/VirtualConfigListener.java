package appsgate.lig.virtual.adapter.listener;
...
import appsgate.lig.virtual.adapter.VirtualAdapter;

public class VirtualConfigListener implements ConfigListener {
	
	private VirtualAdapter adapter;
	
	
	public VirtualConfigListener(VirtualAdapter adapter) {
		super();
	 ...
	 */
	private static Logger logger = LoggerFactory.getLogger(VirtualConfigListener.class);
	...
	public void onReceivedConfig(String cmd, JSONObject obj) {
		...
		try {
			...
			if (cmd.equalsIgnoreCase("addVirtualObject")) {
				...
				adapter.addVirtualObject(obj.getString("type"), properties);

			} else if (cmd.equalsIgnoreCase("removeVirtualObject")) {
				adapter.removeVirtualObject(obj.getString("objectId"));

			} else if (cmd.equalsIgnoreCase("getVirtualObjectList")) {
				JSONArray list = adapter.getVirtualObjectlist();
				adapter.sendResponse(obj.getInt("clientId"), "virtualObjectList", list);
			}
