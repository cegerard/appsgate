...
public class NewBSONDecoder implements BSONDecoder {
    ...
    @Override
    public int decode(final InputStream pIn, final BSONCallback pCallback)
	    throws IOException {
	...
	case NUMBER: {
	    _callback.gotDouble(name,
		    Double.longBitsToDouble(Bits.readLong(_data, _pos)));
	    _pos += 8;
	    return true;
	}
	...
	try {
	    final String str = new String(_data, _pos, (length - 1),
		    DEFAULT_ENCODING);
	    _pos += length;
	    return str;
