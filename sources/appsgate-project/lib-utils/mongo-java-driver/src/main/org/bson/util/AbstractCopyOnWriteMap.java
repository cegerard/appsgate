 ...
 *            The value type
 * @param <M>
 *            the internal {@link Map} or extension for things like sorted and
 *            navigable maps.
 */
@ThreadSafe
abstract class AbstractCopyOnWriteMap<K, V, M extends Map<K, V>> implements
    ...
    private class EntrySet extends CollectionView<Entry<K, V>> implements
	    Set<Map.Entry<K, V>> {
	...
	public enum Type {
	    STABLE {
		@Override
		<K, V, M extends Map<K, V>> View<K, V> get(
			final AbstractCopyOnWriteMap<K, V, M> host) {
	    ...
	    abstract <K, V, M extends Map<K, V>> View<K, V> get(
		    AbstractCopyOnWriteMap<K, V, M> host);
	}
     ...
     * @param viewType
     *            for writable or read-only key, value and entrySet views
     */
    protected <N extends Map<? extends K, ? extends V>> AbstractCopyOnWriteMap(
	    final N map, final View.Type viewType) {
	...
	try {
	    if (!delegate.containsKey(key)
		    || !equals(oldValue, delegate.get(key))) {
		return false;
	    }
	    final M map = copy();
