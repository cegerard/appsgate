...
/**
 * Basic implementation of BSONDecoder interface that creates BasicBSONObject
 * instances.
 * 
 * <h3>Migration instructions</h3> In driver versions before <b>2.12</b>
 * {@code BasicBSONDecoder} exposed several protected members to its subclasses: <br/>
 * <br/>
 * <b>Fields:</b>
 * <ul>
 ...
 * <br/>
 * 
 * <h4>Solution 1: Custom {@link BSONCallback} implementation</h4> With
 * callbacks you can handle the process of creating objects from bytes in BSON
 * format.
 * <p>
 * For example to get away from overriging <b>
 * {@code BasicBSONDecoder._binary(String)}</b> you can use the following piece
 * of code:
 * </p>
 * 
 * <pre>
 ...
 * 
 * <h4>Solution 2: Custom {@link BSONDecoder} implementation</h4> If you need to
 * customize byte-level decoding at the lowest layer you have to provide you own
 * implementation of the {@link BSONDecoder} interface. <br/>
 * Please check <a href="http://bsonspec.org/">http://bsonspec.org/</a> for more
 * information.
 * 
 */
public class BasicBSONDecoder implements BSONDecoder {
    /**
     * @deprecated This class should not be a part of API. Please see the
     *             class-level documentation for a migration instructions.
     */
    @Deprecated
	...
	/**
	 * ensure that there are num bytes to read _pos is where to start
	 * reading from
	 * 
	...
	protected int _need(final int num) throws IOException {
	    ...
	    // System.out.println( "p: " + _pos + " l: " + _len + " want: " +
	    // num );
	    ...
	    int maxToRead = Math.min(_max - _read - remaining,
		    _inputBuffer.length - _len);
	    while (maxToRead > 0) {
	...
	public String readCStr() throws IOException {
		...
		return (out != null) ? out : new String(_random, 0, 1,
			DEFAULT_ENCODING);
	    }
	...
	public String readUTF8String() throws IOException {
	    ...
	    // this is just protection in case it's corrupted, to avoid huge
	    // strings
		...
		return new String(_inputBuffer, _need(size), size - 1,
			DEFAULT_ENCODING);
	    }
    ...
    /**
     * @deprecated This field should not be a part of API. Please see the
     *             class-level documentation for a migration instructions.
     */
    @Deprecated
    ...
    private byte[] _random = new byte[1024]; // has to be used within a single
					     // function
     ...
     * @throws IOException
     * 
     * @deprecated This method should not be a part of API. Please see the
     *             class-level documentation for a migration instructions.
     */
    @Deprecated
	...
	case B_BINARY:
		...
		throw new IllegalArgumentException(
			"bad data size subtype 2 len: " + len + " totalLen: "
				+ totalLen);
	    }
	...
	case B_UUID:
	    if (totalLen != 16) {
		throw new IllegalArgumentException(
			"bad data size subtype 3 len: " + totalLen + " != 16");
	    }
	...
	try {
	    ...
	    if (_in.numRead() != len) {
		throw new IllegalArgumentException(
			"bad data.  lengths don't match read:" + _in.numRead()
				+ " != len:" + len);
	    }
	...
	if (read != len) {
	    // throw new IllegalArgumentException(
	    // "bad data.  lengths don't match " + read + " != " + len );
	}
	...
	case OID:
	    ...
	    _callback.gotObjectId(
		    name,
		    new ObjectId(_in.readIntBE(), _in.readIntBE(), _in
			    .readIntBE()));
	    break;
	...
	case REF:
	    ...
	    ObjectId theOID = new ObjectId(_in.readInt(), _in.readInt(),
		    _in.readInt());
	...
	case CODE_W_SCOPE:
	    _in.readInt();
	    _callback.gotCodeWScope(name, _in.readUTF8String(),
		    _readBasicObject());
	...
	default:
	    throw new UnsupportedOperationException(
		    "BSONDecoder doesn't understand type : " + type + " name: "
			    + name);
	}
