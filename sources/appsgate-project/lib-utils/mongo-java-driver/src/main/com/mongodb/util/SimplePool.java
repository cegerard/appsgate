...
public abstract class SimplePool<T> {
    ...
    /**
     * Creates a new object of this pool's type. Implementations should throw a
     * runtime exception if unable to create.
     * 
	...
	try {
		...
		throw new IllegalStateException(
			"null pool members are not allowed");
	    }
	    return newMember;
    ...
    /**
     * call done when you are done with an object form the pool if there is room
     * and the object is ok will get added
     * 
     * @param t
     *            Object to add
     */
	...
	synchronized (this) {
		...
		throw new RuntimeException(
			"trying to put something back in the pool wasn't checked out");
	    }
     ...
     * 
     * @param waitTime
     *            negative - forever 0 - return immediately no matter what
     *            positive ms to wait
     * @return An object from the pool, or null if can't get one in the given
     *         waitTime
     */
    ...
    /**
     * Pick a member of {@code _avail}. This method is called with a lock held
     * on {@code _avail}, so it may be used safely.
     * 
     * @param recommended
     *            the recommended member to choose.
     * @param couldCreate
	...
	buf.append("pool: ").append(_name).append(" maxToKeep: ").append(_size)
		.append(" avail ").append(_avail.size()).append(" out ")
		.append(_out.size());
	return buf.toString();
    }
}
