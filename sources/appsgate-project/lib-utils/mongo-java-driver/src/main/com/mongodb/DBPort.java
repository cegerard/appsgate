...
/**
 * represents a Port to the database, which is effectively a single connection
 * to a server Methods implemented at the port level should throw the raw
 * exceptions like IOException, so that the connector above can make appropriate
...
public class DBPort {
	...
	GSSAPIAuthenticator(final Mongo mongo, final MongoCredential credentials) {
	    ...
	    if (!this.credential.getMechanism().equals(
		    MongoCredential.GSSAPI_MECHANISM)) {
		throw new MongoException("Incorrect mechanism: "
			+ this.credential.getMechanism());
	    }
	}
	...
	// props.put(Sasl.CREDENTIALS,
	// getGSSCredential(credential.getUserName()));
	//
	// return Sasl.createSaslClient(new String[]{GSSAPI_MECHANISM},
	// credential.getUserName(),
	// credential.getMechanismProperty(SERVICE_NAME_KEY,
	...
	// throw new MongoException("Exception initializing GSSAPI credentials",
	// e);
	// }
	// }
	...
	// private GSSCredential getGSSCredential(String userName) throws
	// GSSException {
	...
	public CommandResult authenticate() {
		...
		CommandResult res = runCommand(db,
			NativeAuthenticationHelper.getNonceCommand());
		res.throwOnError();

		res = runCommand(
			db,
			NativeAuthenticationHelper.getAuthCommand(
				credential.getUserName(),
		...
		throw new MongoException.Network(
			"IOException authenticating the connection", e);
	    }
	}
    }
	...
	// String[]{MongoCredential.PLAIN_MECHANISM}, credential.getUserName(),
	// DEFAULT_PROTOCOL, serverAddress().getHost(), null, new
	// CallbackHandler() {
	// @Override
	...
	public CommandResult authenticate() {
	    ...
	    throw new MongoException(
		    "SASL protocol error: no client response to challenge");
	    ...
	    // response = saslClient.evaluateChallenge((byte[])
	    // res.get("payload"));
	    //
	    // if (response == null) {
	    // throw new
	    // MongoException("SASL protocol error: no client response to challenge");
	    // }
	    //
	    ...
	    // } catch (IOException e) {
	    // throw new
	    // MongoException.Network("IOException authenticating the connection",
	    // e);
	    // } finally {
	    // try {
	    // saslClient.dispose();
	...
	private CommandResult sendSaslContinue(final int conversationId,
		final byte[] outToken) throws IOException {
	    DB adminDB = getDatabase();
	    DBObject cmd = new BasicDBObject("saslContinue", 1).append(
		    "conversationId", conversationId).append("payload",
		    outToken);
	...
	private CommandResult sendSaslStart(final byte[] outToken)
		throws IOException {
	    DBObject cmd = new BasicDBObject("saslStart", 1).append(
		    "mechanism", getMechanismName()).append("payload",
	...
	private DBObject getAuthCommand() {
	    return new BasicDBObject("authenticate", 1).append("user",
		    credential.getUserName()).append("mechanism",
		    MongoCredential.MONGODB_X509_MECHANISM);
	}
    }
    ...
    final Set<String> authenticatedDatabases = Collections
	...
	_logger = Logger.getLogger(_rootLogger.getName() + "."
		+ addr.toString());
	...
	do {
		...
		_socket.connect(_addr.getSocketAddress(),
		...
		_logger.log(Level.WARNING, "Exception connecting to "
			...
			+ " ms.  Will retry after sleeping for " + sleepTime
			+ " ms.");
    ...
    Response call(OutMessage msg, DBCollection coll, DBDecoder decoder)
	    throws IOException {
	return go(msg, coll, false, decoder);
    }

    void checkAuth(Mongo mongo) throws IOException {
	// get the difference between the set of credentialed databases and the
	// set of authenticated databases on this connection
	Set<String> unauthenticatedDatabases = new HashSet<String>(mongo
		.getAuthority().getCredentialsStore().getDatabases());
	unauthenticatedDatabases.removeAll(authenticatedDatabases);

	for (String databaseName : unauthenticatedDatabases) {
	    authenticate(mongo,
		    mongo.getAuthority().getCredentialsStore()
			    .get(databaseName));
	}
    }
	...
	if (res.size() > 1) {
	    throw new MongoInternalException("something is wrong.  size:"
		    + res.size());
	}
	...
	if (data == null) {
	    throw new MongoInternalException(
		    "something is wrong, no command result");
	}
    ...
    synchronized private Response findOne(DB db, String coll, DBObject q)
	    throws IOException {
	OutMessage msg = OutMessage.query(db.getCollection(coll), 0, 0, -1, q,
		null);
	try {
    ...
    private synchronized Response go(OutMessage msg, DBCollection coll,
	    boolean forceResponse, DBDecoder decoder) throws IOException {
	...
	try {
	    ...
	    return new Response(_sa, coll, _in, (decoder == null ? _decoder
		    : decoder));
    ...
    private synchronized Response go(OutMessage msg, DBCollection coll,
	    DBDecoder decoder) throws IOException {
	return go(msg, coll, false, decoder);
    }
    ...
    synchronized CommandResult tryGetLastError(DB db, long last,
	    WriteConcern concern) throws IOException {
	if (last != _calls.get()) {
	    return null;
	}
