...
/**
 * Database API This cannot be directly instantiated, but the functions are
 * available through instances of Mongo.
 * 
...
public class DBApiLayer extends DB {
	...
	@Override
	Iterator<DBObject> __find(DBObject ref, DBObject fields, int numToSkip,
		int batchSize, int limit, int options, ReadPreference readPref,
		DBDecoder decoder) {

	    return __find(ref, fields, numToSkip, batchSize, limit, options,
		    readPref, decoder, DefaultDBEncoder.FACTORY.create());
	}
	...
	Iterator<DBObject> __find(DBObject ref, DBObject fields, int numToSkip,
	    ...
	    OutMessage query = OutMessage.query(this, options, numToSkip,
		    chooseBatchSize(batchSize, limit, 0), ref, fields,
		    readPref, encoder);

	    Response res = _connector.call(_db, this, query, null, 2, readPref,
		    decoder);
	...
	public void createIndex(final DBObject keys, final DBObject options,
	    ...
	    DBApiLayer.this.doGetCollection("system.indexes").insert(
		    Arrays.asList(full), false, WriteConcern.SAFE, encoder);
	}
	...
	protected WriteResult insert(List<DBObject> list, boolean shouldApply,
		com.mongodb.WriteConcern concern, DBEncoder encoder) {
		...
		for (; cur < list.size(); cur++) {
		    ...
		    // limit for batch insert is 4 x maxbson on server, use 2 x
		    // to be safe
	...
	@Override
	public WriteResult update(DBObject query, DBObject o, boolean upsert,
		boolean multi, com.mongodb.WriteConcern concern,
		DBEncoder encoder) {
	    ...
	    if (willTrace()) {
		trace("update: " + _fullNameSpace + " " + JSON.serialize(query)
			+ " " + JSON.serialize(o));
	    }

	    OutMessage om = OutMessage.update(this, encoder, upsert, multi,
		    query, o);
	...
	final ServerAddress _host; // host where first went. all subsequent have
				   // to go there
	...
	// This allows us to easily enable/disable finalizer for cleaning up
	// un-closed cursors
	private final OptionalFinalizer _optionalFinalizer;

	Result(MyCollection coll, Response res, int batchSize, int limit,
	    ...
	    _decoder = decoder;
	    init(res);
	    // Only enable finalizer if cursor finalization is enabled and there
	    // is actually a cursor that needs killing
	    _optionalFinalizer = _mongo.getMongoOptions()
		    .isCursorFinalizerEnabled() && res.cursor() != 0 ? new OptionalFinalizer()
		    : null;
	}
	...
	public boolean hasNext() {
		...
		if (!hasNext) {
			...
			// if server does not support await, driver must sleep
			// to avoid busy loop
	...
	private void init(Response res) {
	    throwOnQueryFailure(res, _curResult == null ? 0
		    : _curResult._cursor);
    ...
    static final Level TRACE_LEVEL = Boolean.getBoolean("DB.TRACE") ? Level.INFO
	    : Level.FINEST;
	...
	if ((res._flags & Bytes.RESULTFLAG_ERRSET) > 0) {
		...
		throw new MongoExecutionTimeoutException(
			ServerError.getCode(errorDocument), ServerError.getMsg(
				errorDocument, null));
	    } else {
		throw new MongoException(ServerError.getCode(errorDocument),
    ...
    /**
     * @param force
     *            true if should clean regardless of number of dead cursors
	...
	OutMessage om = OutMessage.killCursors(_mongo,
		Math.min(NUM_CURSORS_PER_BATCH, all.size()));
	...
	for (Long l : all) {
		...
		om = OutMessage.killCursors(_mongo, Math.min(
			NUM_CURSORS_PER_BATCH, all.size() - totalSoFar));
		soFar = 0;
	    }
	}
