...
/**
 * Base class for classes that manage connections to mongo instances as
 * background tasks.
 */
abstract class ConnectionStatus {
	...
	Node(float pingTime, ServerAddress addr, int maxBsonObjectSize,
		boolean ok) {
	...
	DBPort _port; // we have our own port so we can set different socket
		      // options and don't have to worry about the pool

	float _pingTimeMS = 0;
	int _maxBsonObjectSize;
	ConnectionState _connectionState = Connecting;

	UpdatableNode(final ServerAddress addr, Mongo mongo,
	...
	public CommandResult update() {
		...
		CommandResult res = _port.runCommand(_mongo.getDB("admin"),
			isMasterCmd);
		...
		if (_connectionState != Connected) {
		    ...
		    _pingTimeMS = _pingTimeMS
			    + ((newPingMS - _pingTimeMS) / latencySmoothFactor);
		}

		_maxBsonObjectSize = res.getInt("maxBsonObjectSize",
		...
		getLogger().log(
			Level.FINE,
			"Latency to " + _addr + " actual=" + newPingMS
				+ " smoothed=" + _pingTimeMS);
		...
		if (_connectionState != Unconnected) {
		    ...
		    getLogger().log(Level.WARNING,
			    String.format("Server seen down: %s", _addr), e);
		}
		return null;
	    }
	}
    ...
    protected static final DBObject isMasterCmd = new BasicDBObject("ismaster",
	    1);

    static {
	updaterIntervalMS = Integer.parseInt(System.getProperty(
		"com.mongodb.updaterIntervalMS", "5000"));
	updaterIntervalNoMasterMS = Integer.parseInt(System.getProperty(
		"com.mongodb.updaterIntervalNoMasterMS", "10"));
	mongoOptionsDefaults.connectTimeout = Integer.parseInt(System
		.getProperty("com.mongodb.updaterConnectTimeoutMS", "20000"));
	mongoOptionsDefaults.socketTimeout = Integer.parseInt(System
		.getProperty("com.mongodb.updaterSocketTimeoutMS", "20000"));
	latencySmoothFactor = Float.parseFloat(System.getProperty(
		"com.mongodb.latencySmoothFactor", "4"));
    }
    ...
    /**
     * Ensures that we have the current master, if there is one. If the current
     * snapshot of the replica set has no master, this method waits one cycle to
     * find a new master, and returns it if found, or null if not.
     * 
