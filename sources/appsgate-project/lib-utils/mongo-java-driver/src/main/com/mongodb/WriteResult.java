...
public class WriteResult {
     ...
     * @return
     * @throws MongoException
     * @deprecated Please invoke write operation with appropriate
     *             {@code WriteConcern} and then use {@link #getLastError()}
     *             method.
     */
    @Deprecated
	...
	if (_lastErrorResult != null) {
	    ...
	    if (concern == null
		    || (_lastConcern != null && _lastConcern.getW() >= concern
			    .getW())) {
		return _lastErrorResult;
	    }
	}
	...
	if (_port != null) {
		...
		_lastErrorResult = _port.tryGetLastError(_db, _lastCall,
			(concern == null) ? new WriteConcern() : concern);
	    ...
	    throw new IllegalStateException(
		    "Don't have a port to obtain a write result, and existing one is not good enough.");
	}
