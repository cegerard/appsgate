...
public final class StringParseUtil {
    ...
    /**
     * Turns a string into a boolean value and returns a default value if
     * unsuccessful.
     * 
     * @param s
     ...
     *            the default value
     * @param lastIdx
     *            sets lastIdx[0] to the index of the last digit
     * @param allowNegative
     ...
     * Double.parseDouble to parse s "strictly". i.e. if it's neither a double
     * or an integer, fail.
     * 
     * @param s
     *            the string to convert
	...
	// parse hex
	    ...
	    if (s.length() > 17) {
		throw new RuntimeException("Can't handle a number this big: "
			+ s);
	    }
	    ...
	    if (s.length() > 9) {
		return coef
			* Long.valueOf(s.substring((int) (coef * -.5 + 2.5)),
				16);
	    }
	    return coef
		    * Integer
			    .valueOf(s.substring((int) (coef * -.5 + 2.5)), 16);
	}
