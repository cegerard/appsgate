...
class OutMessage extends BasicBSONEncoder {
    ...
    enum OpCode {
	OP_UPDATE(2001), OP_INSERT(2002), OP_QUERY(2004), OP_GETMORE(2005), OP_DELETE(
		2006), OP_KILL_CURSORS(2007);
    ...
    static OutMessage getMore(DBCollection collection, long cursorId,
	    int batchSize) {
    ...
    public static OutMessage insert(final DBCollection collection,
	    final DBEncoder encoder, WriteConcern concern) {
    ...
    static OutMessage query(DBCollection collection, int options,
	    int numToSkip, int batchSize, DBObject query, DBObject fields) {
	return query(collection, options, numToSkip, batchSize, query, fields,
	...
	return query(collection, options, numToSkip, batchSize, query, fields,
		readPref, DefaultDBEncoder.FACTORY.create());
    }

    static OutMessage query(DBCollection collection, int options,
	    int numToSkip, int batchSize, DBObject query, DBObject fields,
	    ReadPreference readPref, DBEncoder enc) {
	OutMessage om = new OutMessage(collection, enc, query, options,
		readPref);
    ...
    public static OutMessage remove(final DBCollection collection,
	    final DBEncoder encoder, final DBObject query) {
    ...
    public static OutMessage update(final DBCollection collection,
	    final DBEncoder encoder, final boolean upsert, final boolean multi,
	    final DBObject query, final DBObject o) {
    ...
    private volatile int _numDocuments; // only one thread will modify this
					// field, so volatile is sufficient
					// synchronization

    private OutMessage(final DBCollection collection, final DBEncoder enc,
	    final DBObject query, final int options,
	    final ReadPreference readPref) {
	this(collection, collection.getDB().getMongo(), OpCode.OP_QUERY, enc,
		query, options, readPref);
    }

    private OutMessage(final DBCollection collection, final Mongo m,
	    final OpCode opCode, final DBEncoder enc) {
	this(collection, m, opCode, enc, null, -1, null);
    }

    private OutMessage(final DBCollection collection, final Mongo m,
	    OpCode opCode, final DBEncoder enc, final DBObject query,
    ...
    private OutMessage(final DBCollection collection, final OpCode opCode,
	    final DBEncoder enc, final DBObject query) {
	this(collection, collection.getDB().getMongo(), opCode, enc, query, 0,
		null);
    }
	...
	if (objectSize > Math.max(_mongo.getConnector().getMaxBsonObjectSize(),
    ...
    private void writeQuery(final DBObject fields, final int numToSkip,
	    final int batchSize) {
	...
	if (keys.size() == 1 && keys.iterator().next().equals("_id")
		&& _query.get(keys.iterator().next()) instanceof ObjectId) {
	    writeInt(1);
	} else {
	    writeInt(0);
	}
    ...
    private void writeUpdate(final boolean upsert, final boolean multi,
	    final DBObject query, final DBObject o) {
	writeInt(0); // reserved
	writeCString(_collection.getFullName());
