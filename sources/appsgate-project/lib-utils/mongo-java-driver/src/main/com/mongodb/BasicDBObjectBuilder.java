...
/**
 * utility for building complex objects example:
 * BasicDBObjectBuilder.start().add( "name" , "eliot" ).add( "number" , 17
 * ).get()
 */
public class BasicDBObjectBuilder {
    ...
    /**
     * creates an new empty object and inserts it into the current object with
     * the given key. The new child object becomes the active one.
     * 
     * @param key
     * @return returns itself so you can chain
     */
