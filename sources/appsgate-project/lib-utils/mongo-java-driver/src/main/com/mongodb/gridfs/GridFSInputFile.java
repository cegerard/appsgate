...
public class GridFSInputFile extends GridFSFile {
	...
	public void write(byte[] b, int off, int len) throws IOException {
		...
		System.arraycopy(b, offset, _buffer, _currentBufferPosition,
			toCopy);
    ...
    /**
     * Marks the data as fully written. This needs to be called before
     * super.save()
     */
    ...
    protected DBObject createChunk(Object id, int currentChunkNumber,
	    byte[] writeBuffer) {
	return BasicDBObjectBuilder.start().add("files_id", id)
		.add("n", currentChunkNumber).add("data", writeBuffer).get();
    }

    /**
     * After retrieving this {@link java.io.OutputStream}, this object will be
     * capable of accepting successively written data to the output stream. To
     * completely persist this GridFS object, you must finally call the
     * {@link java.io.OutputStream#close()} method on the output stream. Note
     * that calling the save() and saveChunks() methods will throw Exceptions
     * once you obtained the OutputStream.
     * 
	...
	if (_outputStream != null) {
	    throw new MongoException(
		    "cannot mix OutputStream and regular save()");
	}

	// note that chunkSize only changes _chunkSize in case we actually save
	// chunks
	...
	// only finish data, do not write file, in case one wants to change
	// metadata
	_finishData();
