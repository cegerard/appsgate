...
/**
 * This class enables to retrieve a GridFS file metadata and content. Operations
 * include: - writing data to a file on disk or an OutputStream - getting each
 * chunk as a byte array - getting an InputStream to stream the data into
 * 
 * @author antoine
 */
public class GridFSDBFile extends GridFSFile {
	...
	public long skip(long numBytesToSkip) throws IOException {
		...
		// short-circuit here
		// Don't count those extra bytes to skip in with the return
		// value
		return 0;
	    }
	...
	DBObject chunk = _fs._chunkCollection.findOne(BasicDBObjectBuilder
		.start("files_id", _id).add("n", i).get());
	if (chunk == null) {
	    throw new MongoException("can't find a chunk!  file id: " + _id
		    + " chunk: " + i);
	}
