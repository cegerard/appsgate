...
public abstract class TaggableReadPreference extends ReadPreference {
	...
	NearestReadPreference(DBObject firstTagSet,
	...
	PrimaryPreferredReadPreference(DBObject firstTagSet,
		DBObject... remainingTagSets) {
	    super(firstTagSet, remainingTagSets);
	}
    ...
    /**
     * Read from secondary if available, otherwise from primary, irrespective of
     * tags.
     * 
     * @author breinero
     */
    static class SecondaryPreferredReadPreference extends
	    SecondaryReadPreference {
	SecondaryPreferredReadPreference() {
	}

	SecondaryPreferredReadPreference(DBObject firstTagSet,
		DBObject... remainingTagSets) {
	    super(firstTagSet, remainingTagSets);
	}
	...
	SecondaryReadPreference(DBObject firstTagSet,
		DBObject... remainingTagSets) {
	    super(firstTagSet, remainingTagSets);
	}
	...
	ReplicaSetStatus.ReplicaSetNode getNode(ReplicaSetStatus.ReplicaSet set) {
	    ...
	    for (DBObject curTagSet : _tags) {
		List<ReplicaSetStatus.Tag> tagList = getTagListFromDBObject(curTagSet);
		ReplicaSetStatus.ReplicaSetNode node = set
			.getASecondary(tagList);
		if (node != null) {
		    return node;
		}
	    }
    ...
    private static List<ReplicaSetStatus.Tag> getTagListFromDBObject(
	    final DBObject curTagSet) {
	List<ReplicaSetStatus.Tag> tagList = new ArrayList<ReplicaSetStatus.Tag>();
	for (String key : curTagSet.keySet()) {
	    tagList.add(new ReplicaSetStatus.Tag(key, curTagSet.get(key)
		    .toString()));
	}
	return tagList;
    }
