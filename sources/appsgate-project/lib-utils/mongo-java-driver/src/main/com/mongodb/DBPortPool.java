...
public class DBPortPool extends SimplePool<DBPort> {
    ...
    /**
     * @deprecated This class will be dropped in 3.x versions. Please catch
     *             {@link MongoTimeoutException} instead.
     */
    @Deprecated
	...
	final Map<ServerAddress, DBPortPool> _pools = Collections
		.synchronizedMap(new HashMap<ServerAddress, DBPortPool>());
	...
	void close() {
		...
		for (DBPortPool p : _pools.values()) {
			...
			if (MBeanServerFactory.getMBeanServer()
				.isRegistered(on)) {
			    MBeanServerFactory.getMBeanServer()
				    .unregisterMBean(on);
			}
		    } catch (JMException e) {
			Bytes.LOGGER.log(Level.WARNING,
				"jmx de-registration error, continuing", e);
		    }
		}
	    }
	}
	...
	private String createObjectName(ServerAddress addr) {
	    String name = "com.mongodb:type=ConnectionPool,host="
		    + addr.toString().replace(":", ",port=") + ",instance="
		    + _serial;
	...
	DBPortPool get(ServerAddress addr) {
		...
		try {
			...
			Bytes.LOGGER
				.log(Level.INFO,
					"multiple Mongo instances for same host, jmx numbers might be off");
		    }
		    ...
		    Bytes.LOGGER.log(Level.WARNING, "JMX registration error: "
			    + e + "\nContinuing...");
		}
	    }
    ...
    /**
     * @deprecated This class will be dropped in 3.x versions. Please catch
     *             {@link MongoClientException} instead.
     */
    @Deprecated
    ...
    /**
     * @deprecated This class will be dropped in 3.x versions. Please catch
     *             {@link MongoWaitQueueFullException} instead.
     */
    @Deprecated
	...
	_options = options;
	_addr = addr;
	_waitingSem = new Semaphore(_options.connectionsPerHost
	...
	if (!_waitingSem.tryAcquire()) {
	    throw new SemaphoresOut(_options.connectionsPerHost
		    * _options.threadsAllowedToBlockForConnectionMultiplier);
	}
	...
	for (DBPort port : _out) {
	    inUseConnectionInfoList.add(new InUseConnectionBean(port,
		    currentNanoTime));
	}
	return inUseConnectionInfoList
		.toArray(new InUseConnectionBean[inUseConnectionInfoList.size()]);
    }
	...
	return new ConnectionPoolStatisticsBean(getTotal(), getInUse(),
		getInUseConnections());
    }
	...
	Bytes.LOGGER.log(Level.WARNING, "emptying DBPortPool to "
		+ getServerAddress() + " b/c of error", e);
