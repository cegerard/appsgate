...
/**
 * An effectively immutable store of credentials to mongo servers. It enforces
 * the invariant that there can be at most one credentials for each database. It
 * allows at most one credentials with a null database.
 * 
 * There is still a package-protected method to add a new credentials to the
 * store, but it's only there to support DB.authenticate, which allows you to
 * add new credentials at any point during the life of a MongoClient.
 * 
 * @since 2.11.0
 */
@ThreadSafe
class MongoCredentialsStore {
     ...
     * 
     * @param credentials
     *            A single credentials, which may be null.
     */
     ...
     *             if there already exist different credentials for the same
     *             database
     */
    synchronized void add(MongoCredential credentials) {
	MongoCredential existingCredentials = credentialsMap.get(credentials
		.getSource());
	...
	if (existingCredentials != null) {
	    ...
	    throw new IllegalArgumentException(
		    "Can't add more than one credentials for the same database");
	}

	credentialsMap.put(credentials.getSource(), credentials);
	allDatabasesWithCredentials = new HashSet<String>(
     ...
     * 
     * @param database
     *            the database. This can be null, to get the credentials with
     *            the null database.
     * @return the credentials for the given database. Can be null if not are
     *         stored.
     */
     ...
     * 
     * @return an unmodifiable set of database names. Can contain the null
     *         string.
     */
