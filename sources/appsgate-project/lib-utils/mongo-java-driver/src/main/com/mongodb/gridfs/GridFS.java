 ...
 * 
 * <a href="http://www.mongodb.org/display/DOCS/GridFS+Specification">GridFS 1.0
 * spec</a>
 * 
 * @dochub gridfs
 */
public class GridFS {
     ...
     * 
     * @deprecated You can calculate max chunkSize with a similar formula
     *             {@link com.mongodb.MongoClient#getMaxBsonObjectSize()} -
     *             500*1000. Please ensure that you left enough space for
     *             metadata (500kb is enough).
     */
    @Deprecated
	...
	try {
		...
		_filesCollection.ensureIndex(BasicDBObjectBuilder.start()
			.add("filename", 1).add("uploadDate", 1).get());
	    }
	    if (_chunkCollection.count() < 1000) {
		_chunkCollection.ensureIndex(
			BasicDBObjectBuilder.start().add("files_id", 1)
				.add("n", 1).get(), BasicDBObjectBuilder
				.start().add("unique", true).get());
	    }
	} catch (MongoException e) {
	    LOGGER.info(String
		    .format("Unable to ensure indices on GridFS collections in database %s",
			    db.getName()));
	}
    ...
    /**
     * creates a file entry. After calling this method, you have to call save()
     * on the GridFSInputFile file
     * 
     * @param data
     *            the file's data
     * @return
