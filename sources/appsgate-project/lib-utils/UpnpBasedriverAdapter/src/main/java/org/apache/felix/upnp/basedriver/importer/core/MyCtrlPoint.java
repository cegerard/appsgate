    ...
    public void newEventArrived(String uuid, long seq, PropertyList props) {
        ...
        if (service != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                String varName = prop.getName();
                String varValue = prop.getValue();
                StateVariable stateVariable = service.getStateVariable(varName);
                if (stateVariable == null) {
                    Activator.logger.WARNING("No value found for varName: " + varName + " for service[" + service.getServiceType() + ":" + service.getServiceID() + "]");
                } else {
