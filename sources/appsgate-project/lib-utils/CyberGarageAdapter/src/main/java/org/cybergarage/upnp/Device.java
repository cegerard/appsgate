		...
		if ((baseURLStr == null) || (baseURLStr.length() <= 0)) {
			if ((locationURLStr != null) && (0 < locationURLStr.length())) {
				if (!locationURLStr.endsWith("/") || !urlString.startsWith("/")) {
					...
					// Ugly way to retrieve real location when URL contains a path
					if (!locationURLStr.endsWith("/")) {
						int index = locationURLStr.indexOf("//");
						index += 2;
						while (locationURLStr.indexOf("/", index) == (index + 1))
							index++;
						// now the separator of protocol is skipped, next slash
						// begins relative position
						index = locationURLStr.indexOf("/", index);
						if (index > 0)
							locationURLStr = locationURLStr.substring(0, index);
					}
