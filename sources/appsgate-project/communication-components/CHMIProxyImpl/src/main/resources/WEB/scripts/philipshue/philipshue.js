	...
	var returnedModule = function () {
		...
		*/
		this.messageHandler = function messageHandler(message) {
			if (message.hasOwnProperty("hueConfDevices")) {
				...
				for(light in lightsArray) {
					...
					chmi.addNotifHandler(lightsArray[light].lightId, this.notificationHandler);
				}
				...
				element.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().triggerCmd(\"hue\",\""+lightInfo.lightId+"\", \"\")");
				
				element = document.getElementById("lightstate-sat");
				element.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().triggerCmd(\"sat\",\""+lightInfo.lightId+"\", \"\")");
				
				element = document.getElementById("lightstate-bri");
				element.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().triggerCmd(\"bri\",\""+lightInfo.lightId+"\", \"\")");
				
				element = document.getElementById("lightstate-xy");
				element.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().triggerCmd(\"xy\",\""+lightInfo.lightId+"\",\""+lightInfo.bridgeIp+"\")");
				
				element = document.getElementById("lightstate-ct");
				element.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().triggerCmd(\"ct\",\""+lightInfo.lightId+"\",\""+lightInfo.bridgeIp+"\")");
				
				element = document.getElementById("lightstate-transition");
				element.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().triggerCmd(\"tt\",\""+lightInfo.lightId+"\", \"\")");
				
				element = document.getElementById("lightstate-alert");
				element.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().triggerCmd(\"alert\",\""+lightInfo.lightId+"\", \"\")");
				
				element = document.getElementById("lightstate-effect");
				element.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().triggerCmd(\"effect\",\""+lightInfo.lightId+"\", \"\")");
				
				element = document.getElementById("lightstate-mode");
				element.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().triggerCmd(\"mode\",\""+lightInfo.lightId+"\",\""+lightInfo.bridgeIp+"\")");
				...
				if(hueDiv != null) {
					...
					chmi.sendCmd("{\"getHUEConfDevices\":{}, \"CONFIGURATION\":\"getHUEConfDevices\", \"TARGET\":\"PHILIPSHUE\"}");
					...
					actionTile.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().pushLinkSend(\""+bridge+"\");");
					actionTile.className = "tile square text bg-color-green";
					
					actionTile = document.getElementById("updatesoftwareTile");
					actionTile.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().updatefirmware(\""+bridge+"\");");
					actionTile.className = "tile square text bg-color-green";
					
					actionTile = document.getElementById("findlightserialTile");
					var serial = "";
					actionTile.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().findNewLightSerial(\""+bridge+"\", \""+serial+"\");");
					actionTile.className = "tile square text bg-color-green";
					
					actionTile = document.getElementById("findnewlightsTile");
					actionTile.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().findNewLights(\""+bridge+"\");");
					actionTile.className = "tile square text bg-color-green";
		...
		this.addBridgeTile = function addBridgeTile(bridge) {
			...
			atag.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().goToBridgeDisplay(\""+bridge.ip+"\");")
		...
		this.addLightTile = function addLightTile(light, lights_tile_list, bridgeIp) {
			...
			var atag = lightDiv.childNodes[0];
			if(bridgeIp == "") {
				atag.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().goToLightDisplay(\""+light.bridgeIp+"\", \""+light.bridgeLightId+"\", \"\");");
			}else {
				atag.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().goToLightDisplay(\""+light.bridgeIp+"\", \""+light.bridgeLightId+"\", \""+bridgeIp+"\");");
			}
			var subDiv = atag.childNodes;
			var nbDiv = subDiv.length;
			var currentDiv;
		...
		this.goToBridgeDisplay = function goToBridgeDisplay(bridge) {
			...
			chmi.gotToNextSubMenu("bridge "+bridge, httpRequest.responseText, "javascript:chmi.goToHueSubMenu();");
			...
			if(status.indexOf("OK") != -1) {
				...
				actionTile = document.getElementById("findlightserialTile");
				actionTile.setAttribute("onclick", "javascript:chmi.getWebSocket().getHue().findNewLightSerial(\""+bridge+"\");");
		...
		this.goToLightDisplay = function goToLightDisplay(bridge, light, fromBridge){
			...
			//Display the next sub menu and update the navigation bar
			if(fromBridge == ""){
				chmi.gotToNextSubMenu("light-"+light, httpRequest.responseText, "chmi.goToHueSubMenu()");
			}else {
				chmi.gotToNextSubMenu("light-"+light, httpRequest.responseText, "javascript:chmi.getWebSocket().getHue().goToBridgeDisplay(\""+fromBridge+"\");");
			}
