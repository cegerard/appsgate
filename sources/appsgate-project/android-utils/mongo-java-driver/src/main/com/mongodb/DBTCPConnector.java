...
/**
 * @deprecated This class is NOT part of the public API. It will be dropped in
 *             3.x releases.
 */
@Deprecated
public class DBTCPConnector implements DBConnector {
	...
	/**
	 * call this method when there is an IOException or other low level
	 * error on port.
	 * 
	 * @param port
	 * @param e
	...
	void error(DBPort port, Exception e) {
	    ...
	    // depending on type of error, may need to close other connections
	    // in pool
	    boolean recoverable = port.getPool().gotError(e);
	    if (!recoverable && _connectionStatus != null
		    && _masterPortPool._addr.equals(port.serverAddress())) {
		ConnectionStatus.Node newMaster = _connectionStatus
			.ensureMaster();
		if (newMaster != null) {
		    setMaster(newMaster);
		}
	...
	DBPort get(boolean keep, ReadPreference readPref,
		...
		if (pinnedRequestPort != null
			&& pinnedRequestPort.serverAddress().equals(hostNeeded)) {
		...
		if (pinnedRequestPort.getPool() == _masterPortPool || !keep) {
		    // if keep is false, it's a read, so we use port even if
		    // master changed
		    return pinnedRequestPort;
		}
		...
		// this may not be best behavior if spec of request is to stick
		// with same server
		...
		if (_masterPortPool == null) {
		    // this should only happen in rare case that no master was
		    // ever found
		    // may get here at startup if it's a read, slaveOk=true, and
		...
		ReplicaSetStatus.ReplicaSet replicaSet = getReplicaSetStatus()._replicaSetHolder
			.get();
		ConnectionStatus.Node node = readPref.getNode(replicaSet);

		if (node == null) {
		    throw new MongoException(
			    "No replica set members available in " + replicaSet
				    + " for "
				    + readPref.toDBObject().toString());
		}
	...
	DBPort getPinnedRequestPortForThread() {
	    return threadHasPinnedRequest() ? pinnedRequestStatusThreadLocal
		    .get().requestPort : null;
	}
	...
	if (type == MongoAuthority.Type.Direct) {
	    ...
	    _connectionStatus = new DynamicConnectionStatus(mongo, mongo
		    .getAuthority().getServerAddresses());
	} else {
    ...
    /**
     * This method is called in case of an IOException. It will potentially
     * trigger a checkMaster() to check the status of all servers.
     * 
     * @param t
	...
	// the replset has at least 1 server up, try to see if should switch
	// master
	// if no server is up, we wont retry until the updater thread finds one
	// this is to cut down the volume of requests/errors when all servers
	// are down
	...
	final DBPort port = _myPort.get(false,
		ReadPreference.primaryPreferred(), null);
    ...
    @Override
    public Response call(DB db, DBCollection coll, OutMessage m,
	    ServerAddress hostNeeded, int retries) {
    ...
    @Override
    public Response call(DB db, DBCollection coll, OutMessage m,
	    ServerAddress hostNeeded, int retries, ReadPreference readPref,
	    DBDecoder decoder) {
	try {
	    return innerCall(db, coll, m, hostNeeded, retries, readPref,
		    decoder);
	} finally {
	    m.doneWithMessage();
	}
	...
	if (_connectionStatus instanceof ReplicaSetStatus) {
	    ...
	    return ((DynamicConnectionStatus) _connectionStatus)
		    .asReplicaSetStatus();
	} else {
	    return null;
	}
	...
	try {
	    CommandResult res = port.runCommand(_mongo.getDB("admin"),
	    ...
	    _logger.log(
		    Level.WARNING,
		    "Exception executing isMaster command on "
			    + port.serverAddress(), e);
	} finally {
	    port.getPool().done(port);
	}
    }
    ...
    private Response innerCall(final DB db, final DBCollection coll,
	    final OutMessage m, final ServerAddress hostNeeded,
	...
	if (readPref == ReadPreference.primary()
		&& m.hasOption(Bytes.QUERYOPTION_SLAVEOK)) {
	...
	_checkClosed();
	// Don't check master on secondary reads unless connected to a replica
	// set
	...
	try {
	    ...
	    retry = retries > 0 && !coll._name.equals("$cmd")
		    ...
		    && _error(ioe, secondaryOk);
	    if (!retry) {
		throw new MongoException.Network("Read operation to server "
			+ port.host() + " failed on database " + db, ioe);
	    }
	...
	if (retry) {
	    return innerCall(db, coll, m, hostNeeded, retries - 1, readPref,
		    decoder);
	}
    ...
    // Be careful when modifying this method, as this method is using the fact
    // that _isMongosDirectConnection
	...
	return _isMongosDirectConnection != null ? _isMongosDirectConnection
		: false;
    }
	...
	try {
		...
		throw new MongoException.Network("Write operation to server "
			+ port.host() + " failed on database " + db, ioe);
	    }
	...
	if (_masterPortPool != null) {
	    _logger.log(Level.WARNING, "Primary switching from "
		    + _masterPortPool.getServerAddress() + " to " + addr);
	}
    ...
    /**
     * Assigns a new DBPortPool for a given ServerAddress. This is used to
     * obtain a new pool when the resolved IP of a host changes, for example.
