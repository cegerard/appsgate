...
public abstract class DB {
    ...
    // cached authentication command result, to return in case of multiple calls
    // to authenticateCommand with the
    // same credentials
	...
	ByteArrayOutputStream bout = new ByteArrayOutputStream(
		username.length() + 20 + passwd.length);
	try {
		...
		if (passwd[i] >= 128) {
		    throw new IllegalArgumentException(
			    "can't handle non-ascii passwords yet");
		}
    ...
    /**
     * Adds privilege documents to the {@code system.users} collection in a
    ...
    /**
     * Authenticates to db with the given credentials. If this method (or
     * {@code authenticateCommand}) has already been called with the same
     * credentials and the authentication test succeeded, this method will
     * return {@code true}. If this method has already been called with
     * different credentials and the authentication test succeeded, this method
     * will throw an {@code IllegalStateException}. If this method has already
     * been called with any credentials and the authentication test failed, this
     * method will re-try the authentication test with the given credentials.
     * 
     * @param username
     ...
     * @throws MongoException
     *             if authentication failed due to invalid user/pass, or other
     *             exceptions like I/O
     * @throws IllegalStateException
     *             if authentiation test has already succeeded with different
     *             credentials
     * @dochub authenticate
     * @see #authenticateCommand(String, char[])
     * @deprecated Please use
     *             {@link MongoClient#MongoClient(java.util.List, java.util.List)}
     *             to create a client, which will authentificate all connections
     *             to server
     */
    @Deprecated
    ...
    /**
     * Authenticates to db with the given credentials. If this method (or
     * {@code authenticate}) has already been called with the same credentials
     * and the authentication test succeeded, this method will return true. If
     * this method has already been called with different credentials and the
     * authentication test succeeded, this method will throw an
     * {@code IllegalStateException}. If this method has already been called
     * with any credentials and the authentication test failed, this method will
     * re-try the authentication test with the given credentials.
     * 
     * @param username
     *            name of user for this database
     * @param password
     ...
     */
    @Deprecated
    public synchronized CommandResult authenticateCommand(String username,
	    char[] password) {
	CommandResultPair commandResultPair = authenticateCommandHelper(
		username, password);
    ...
    private CommandResultPair authenticateCommandHelper(String username,
	    char[] password) {
	MongoCredential credentials = MongoCredential.createMongoCRCredential(
		username, getName(), password);
	if (getAuthenticationCredentials() != null) {
		...
		if (authenticationTestCommandResult != null) {
		    return new CommandResultPair(
			    authenticationTestCommandResult);
		}
	    } else {
     ...
     *            a name of the collection to test for existence
     * @return {@code false} if no collection by that name exists, {@code true}
     *         if a match to an existing collection was found
     * @throws MongoException
     */
     ...
     * {@link DB#command(DBObject, int)} with 0 as query option.
     * 
     * @param cmd
     *            {@code DBObject} representation of the command to be executed
    ...
    /**
     * Executes a database command. This method calls
     * {@link DB#command(com.mongodb.DBObject, int, com.mongodb.DBEncoder) } with
     * 0 as query option.
     * 
     * @param cmd
     *            {@code DBObject} representation of the command to be executed
     * @param encoder
     *            {@link DBEncoder} to be used for command encoding
     * @return result of the command execution
     * @throws MongoException
     * @dochub commands
     */
     ...
     * 
     * @param cmd
     *            {@code DBObject} representation the command to be executed
     * @param options
    ...
    /**
     * Executes a database command. This method calls
     * {@link DB#command(com.mongodb.DBObject, int, com.mongodb.ReadPreference, com.mongodb.DBEncoder) }
     ...
     *            query options to use
     * @param readPrefs
     *            {@link ReadPreference} for this command (nodes selection is
	...
	return command(cmd, options, readPrefs,
     ...
     */
    public CommandResult command(DBObject cmd, int options,
	    ReadPreference readPrefs, DBEncoder encoder) {
	ReadPreference effectiveReadPrefs = getCommandReadPreference(cmd,
		readPrefs);
	cmd = wrapCommand(cmd, effectiveReadPrefs);

	Iterator<DBObject> i = getCollection("$cmd").__find(cmd,
		new BasicDBObject(), 0, -1, 0, options, effectiveReadPrefs,
	...
	DBObject res = i.next();
	ServerAddress sa = (i instanceof Result) ? ((Result) i)
    ...
    /**
     * Executes a database command. This method constructs a simple dbobject and
    ...
    /**
     * Executes a database command. This method constructs a simple dbobject and
     * calls {@link DB#command(com.mongodb.DBObject, int)  }
     * 
     * @param cmd
     *            name of the command to be executed
     * @param options
     ...
     * does not exist, a new collection is created.
     * <p/>
     * Possible options:
     * <ul>
     * <li>
     * <b>capped</b> ({@code boolean}) - Enables a collection cap. False by
     * default. If enabled, you must specify a size parameter.</li>
     * <li>
     * <b>size</b> ({@code int}) - If capped is true, size specifies a maximum
     * size in bytes for the capped collection. When capped is false, you may
     * use size to preallocate space.</li>
     * <li>
     * <b>max</b> ({@code int}) - Optional. Specifies a maximum "cap" in number
     * of documents for capped collections. You must also specify size when
     * specifying max.</li>
     * <p/>
     * </ul>
     * <p/>
     * Note that if the {@code options} parameter is {@code null}, the creation
     * will be deferred to when the collection is written to.
     * 
     * @param name
    ...
    /**
     * Evaluates JavaScript functions on the database server. This is useful if
     * you need to touch a lot of data lightly, in which case network transfer
	...
	return command(BasicDBObjectBuilder.start().add("$eval", code)
		.add("args", args).get());
    }
     ...
     * <p/>
     * This class is NOT part of the public API. Be prepared for non-binary
    ...
    /**
     * Drops this database, deleting the associated data files. Use with
     * caution.
     * 
     * @throws MongoException
     */
    ...
    /**
     * Calls {@link DB#doEval(java.lang.String, java.lang.Object[]) }. If the
     * command is successful, the "retval" field is extracted and returned.
    ...
    /**
     * For testing purposes only - this method forces an error to help test
     * error handling
     * 
     * @throws MongoException
     */
	...
	Iterator<DBObject> i = namespaces.__find(new BasicDBObject(), null, 0,
		0, 0, getOptions(), getReadPreference(), null);
	if (i == null) {
     ...
     * 
     * @param command
     *            the {@link DBObject} representing the command
     * @param requestedPreference
     *            the preference requested by the client.
     * @return the read preference to use for the given command. It will never
     *         return {@code null}.
     * @see com.mongodb.ReadPreference
     */
    ReadPreference getCommandReadPreference(DBObject command,
     ...
     * </code>
     * <p/>
     * The value for errorMessage will be null if no error occurred, or a
     * description otherwise.
     * <p/>
     * Important note: when calling this method directly, it is undefined which
     * connection "getLastError" is called on. You may need to explicitly use a
     * "consistent Request", see {@link DB#requestStart()} For most purposes it
     * is better not to call this method directly but instead use
     * {@link WriteConcern}
     * 
     ...
     * 
     * @param concern
     *            a {@link WriteConcern} to be used while checking for the error
     *            status.
     * @return {@code DBObject} with error and status information
     * @throws MongoException
     ...
     * 
     * @param w
     *            when running with replication, this is the number of servers
     *            to replicate to before returning. A <b>w</b> value of <b>1</b>
     *            indicates the primary only. A <b>w</b> value of <b>2</b>
     *            includes the primary and at least one secondary, etc. In place
     *            of a number, you may also set <b>w</b> to majority to indicate
     *            that the command should wait until the latest write propagates
     *            to a majority of replica set members. If using <b>w</b>, you
     *            should also use <b>wtimeout</b>. Specifying a value for
     *            <b>w</b> without also providing a <b>wtimeout</b> may cause
     *            {@code getLastError} to block indefinitely.
     * @param wtimeout
     *            a value in milliseconds that controls how long to wait for
     *            write propagation to complete. If replication does not
     *            complete in the given timeframe, the getLastError command will
     *            return with an error status.
     * @param fsync
     *            if <b>true</b>, wait for {@code mongod} to write this data to
     *            disk before returning. Defaults to <b>false</b>.
     * @return {@code DBObject} with error and status information
     * @throws MongoException
     * @see {@link DB#getLastError(com.mongodb.WriteConcern) }
     */
    public CommandResult getLastError(int w, int wtimeout, boolean fsync) {
	return command((new com.mongodb.WriteConcern(w, wtimeout, fsync))
		.getCommand());
    }
     ...
     * {@link com.mongodb.DB#resetError()}
     * <p/>
     ...
     * </code>
     * <p/>
     * The value for errorMessage will be null of no error has occurred,
     * otherwise the error message. The value of countOpsBack will be the number
     * of operations since the error occurred.
     * <p/>
     * Care must be taken to ensure that calls to getPreviousError go to the
     * same connection as that of the previous operation. See
     * {@link DB#requestStart()} for more information.
     * 
     * @return {@code DBObject} with error and status information
     * @throws MongoException
     */
     ...
     * 
     * @return {@code ReadPreference} to be used for read operations, if not
    ...
    /**
     * Helper method for calling a 'dbStats' command. It returns storage
     ...
     * 
     * @return {@code WriteConcern} to be used for write operations, if not
     *         specified explicitly
     */
     ...
     * <p/>
     * Following this call and until {@link com.mongodb.DB#requestDone()} is
     * called, all db operations will use the same underlying connection.
     * <p/>
     * This is useful to ensure that operations happen in a certain order with
     * predictable results.
     */
    public abstract void requestStart();

    /**
     * Resets the error memory for this database. Used to clear all errors such
     * that {@link DB#getPreviousError()} will return no error.
     * 
     * @throws MongoException
     */
    ...
    /**
     * Makes this database read-only. Important note: this is a convenience
     * setting that is only known on the client side and not persisted.
     * 
     * @param b
     *            if the database should be read-only
     * @deprecated Avoid making database read-only via this method. Connect with
     *             a user credentials that has a read-only access to a server
     *             instead.
     */
    @Deprecated
    ...
    /**
     * Sets the write concern for this database. It will be used for write
     * operations to any collection in this database. See the documentation for
     * {@link WriteConcern} for more information.
     * 
     * @param concern
     *            {@code WriteConcern} to use
     */
    ...
    // Only append $readPreference meta-operator if connected to a mongos, read
    // preference is not primary
    // or secondary preferred,
    // and command is an instance of BasicDBObject. The last condition is
    // unfortunate, but necessary in case
    // the encoder is not capable of encoding a BasicDBObject
    // Due to issues with compatibility between different versions of mongos,
    // also wrap the command in a
	...
	if (getMongo().isMongosConnection()
		&& !(ReadPreference.primary().equals(readPrefs) || ReadPreference
			.secondaryPreferred().equals(readPrefs))
		&& cmd instanceof BasicDBObject) {
	    cmd = new BasicDBObject("$query", cmd).append(
		    QueryOpBuilder.READ_PREFERENCE_META_OPERATOR,
		    readPrefs.toDBObject());
	}
	return cmd;
    }
}
