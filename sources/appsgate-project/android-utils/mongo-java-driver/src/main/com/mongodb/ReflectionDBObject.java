...
public abstract class ReflectionDBObject implements DBObject {
    ...
    /**
     * Represents a wrapper around the DBObject to interface with the Class
     * fields
     */
	...
	JavaWrapper(Class c) {
		...
		if (!(m.getName().startsWith("get") || m.getName().startsWith(
			"set"))) {
		    continue;
		}
		...
		Class type = m.getName().startsWith("get") ? m.getReturnType()
	...
	public Object get(ReflectionDBObject t, String name) {
		...
		throw new RuntimeException("could not invoke getter for ["
			+ name + "] on [" + _name + "]", e);
	    }
	}
	...
	public Object set(ReflectionDBObject t, String name, Object val) {
		...
		throw new RuntimeException("could not invoke setter for ["
			+ name + "] on [" + _name + "]", e);
	    }
	}
    }
    ...
    private static final Map<Class, JavaWrapper> _wrappers = Collections
	    .synchronizedMap(new HashMap<Class, JavaWrapper>());
    ...
    /**
     * Returns an existing Wrapper instance associated with a class, or creates
     * a new one.
     * 
     * @param c
     * @return
