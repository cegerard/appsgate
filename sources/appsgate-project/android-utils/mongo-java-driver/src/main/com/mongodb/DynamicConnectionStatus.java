...
/**
 * Responsible for dynamically determining whether the list of server address
 * represents a set of mongos server or a replica set. It starts threads that
 * call the ismaster command on every server in the seed list, and as soon as it
 * reaches one determines what type of server it is. It then creates the
 * appropriate ConnectionStatus implementation and forwards all calls to it.
 */
class DynamicConnectionStatus extends ConnectionStatus {
	...
	DynamicNode(final ServerAddress addr, Mongo mongo,
    ...
    private static final Logger logger = Logger
	    .getLogger("com.mongodb.DynamicConnectionStatus");
	...
	if (connectionStatus == null) {
		...
		throw new MongoInterruptedException(
			"Interrupted while waiting for next update to dynamic status",
			e);
	    }
	}
	...
	try {
		...
		executorService.submit(new Runnable() {
		    @Override
		    public void run() {
			DynamicNode node = new DynamicNode(cur, _mongo,
				_mongoOptions);
			try {
				...
				try {
				    ...
				    logger.log(Level.WARNING, "couldn't reach "
					    + node._addr, e);
				}
	    ...
	    // Ignore, as this can happen if a good node is found before all
	    // jobs are submitted and the service has
	    // been shutdown.
	}
    }
	...
	synchronized (this) {
		...
		connectionStatus = new ReplicaSetStatus(_mongo,
			_mongosAddresses);
	    }
	    notifyAll();
	}
