...
class MongosStatus extends ConnectionStatus {
	...
	MongosNode(final ServerAddress addr, Mongo mongo,
	...
	private List<MongosNode> getMongosNodes() {
	    List<MongosNode> mongosNodes = new ArrayList<MongosNode>(
		    _mongosAddresses.size());
		...
		while (!Thread.interrupted()) {
			...
			for (MongosNode cur : mongosNodes) {
				...
				if (bestThisPass == null
					|| (cur._pingTimeMS < bestThisPass._pingTimeMS)) {
				    bestThisPass = cur;
				}
			    }
			}
		    ...
		    int sleepTime = preferred == null ? updaterIntervalNoMasterMS
			    : updaterIntervalMS;
    ...
    private static final Logger logger = Logger
	    .getLogger("com.mongodb.MongosStatus");

    // The current preferred mongos Node to use as the master. This is not
    // necessarily the node that is currently in use.
    // Rather, it's the node that is preferred if there is a problem with the
    // currently in use node.
    ...
    // Gets the current preferred node. If there is no preferred node, wait to
    // get a notification before returning null.
	...
	if (preferred == null) {
		...
		throw new MongoInterruptedException(
			"Interrupted while waiting for next update to mongos status",
			e);
	    }
	}
	return preferred;
	...
	if (bestThisPass == null) {
	    ...
	    preferred = new Node(bestThisPass._pingTimeMS, bestThisPass._addr,
		    bestThisPass._maxBsonObjectSize, bestThisPass.isOk());
	}
	notifyAll();
    }
}