...
/**
 * Utility for constructing Query operation command with query, orderby, hint,
 * explain, snapshot.
 */
class QueryOpBuilder {
     ...
     * 
     * @param dbobj
     *            DBObject to add field to
     * @param field
     *            name of the field
     * @param obj
     *            object to add to the operation. Ignore if <code>null</code>.
     * @param sendEmpty
     *            if <code>true</code> adds obj even if it's empty. Ignore if
     *            <code>false</code> and obj is empty.
     * @return
     */
    private void addToQueryObject(DBObject dbobj, String field, DBObject obj,
	    boolean sendEmpty) {
	if (obj == null) {
	    return;
	}
     ...
     *            name of the field
     * @param obj
     *            Object to be added. Ignore if <code>null</code>
     * @return
     */
	...
	if (hasSpecialQueryFields()) {
	    DBObject queryop = (specialFields == null ? new BasicDBObject()
		    : specialFields);
		...
		queryop.put(READ_PREFERENCE_META_OPERATOR,
			readPref.toDBObject());
	    }
	    if (maxTimeMS > 0) {
