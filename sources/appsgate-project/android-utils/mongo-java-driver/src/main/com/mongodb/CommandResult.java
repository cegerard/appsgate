...
public class CommandResult extends BasicDBObject {
	...
	if (!ok()) { // check for command failure
	    if (getCode() == 50) {
		return new MongoExecutionTimeoutException(getCode(),
			getErrorMessage());
	    } else {
		return new CommandFailureException(this);
	    }
	} else if (hasErr()) { // check for errors reported by getlasterror
			       // command
	...
	throw new MongoInternalException(
		"Unexpected value type for 'ok' field in command result: "
			+ okValue.getClass().getSimpleName());
    }

    /**
     * throws an exception containing the cmd name, in case the command failed,
     * or the "err/code" information
     * 
