 ...
 * 
 * @deprecated This class is no longer in use and should not have been public.
...
public abstract class ThreadPool<T> {
	...
	MyThread() {
	    super(_myThreadGroup, "ThreadPool.MyThread:" + _name + ":"
		    + _threads.size());
	    setDaemon(true);
	    start();
	}
     ...
     *            identifying name
     * @param numThreads
     *            the number of threads allowed in the pool
     */
    ...
    /**
     * Initializes a new thread pool with a given name, number of threads, and
     * queue size.
     * 
     * @param name
     *            identifying name
     ...
     *            the number of threads allowed in the pool
     * @param maxQueueSize
     *            the size of the pool entry queue
     */
