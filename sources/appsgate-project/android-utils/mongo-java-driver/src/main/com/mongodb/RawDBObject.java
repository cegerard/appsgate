...
/**
 * This object wraps the binary object format ("BSON") used for the transport of
 * serialized objects to / from the Mongo database.
 * 
...
public class RawDBObject implements DBObject {
	...
	Element(final int start) {
		...
		break;
	    default:
		throw new RuntimeException("RawDBObject can't size type "
			+ _type);
	    }
	    _size = size;
	}
	...
	Object getObject() {
	    ...
	    case OID:
		return new ObjectId(_buf.getInt(_dataStart),
			_buf.getInt(_dataStart + 4),
			_buf.getInt(_dataStart + 8));
	    case CODE:
	    case CODE_W_SCOPE:
	...
	synchronized (_cStrBuf) {
		...
		if (pos >= _cStrBuf.length) {
		    throw new IllegalArgumentException(
			    "c string too big for RawDBObject.  so far["
				    + new String(_cStrBuf) + "]");
		}
		...
		if (pos + start >= _buf.limit()) {
		    ...
		    throw new IllegalArgumentException(
			    "can't find end of cstring.  start:" + start
				    + " pos: " + pos + " [" + sb + "]");
		}
	    }
