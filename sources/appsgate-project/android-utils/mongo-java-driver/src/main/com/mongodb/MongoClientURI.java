...
/**
 * Represents a <a
 * href="http://www.mongodb.org/display/DOCS/Connections">URI</a> which can be
 * used to create a MongoClient instance. The URI describes the hosts to be used
 * and options.
 * <p>
 ...
 * server address to connect to.</li>
 ...
 * <li>{@code ?options} are connection options. Note that if {@code database} is
 * absent there is still a {@code /} required between the last host and the
 * {@code ?} introducing the options. Options are name=value pairs and the pairs
 * are separated by "&amp;". For backwards compatibility, ";" is accepted as a
 * separator in addition to "&amp;", but should be considered as deprecated.</li>
 * </ul>
 * <p>
 ...
 * </p>
 * <ul>
 * <li>{@code replicaSet=name}: Implies that the hosts given are a seed list,
 * and the driver will attempt to find all members of the set.</li>
 * </ul>
 * <p>
 ...
 * <li>{@code connectTimeoutMS=ms}: How long a connection can take to be opened
 * before timing out.</li>
 * <li>{@code socketTimeoutMS=ms}: How long a send or receive on a socket can
 * take before timing out.</li>
 * </ul>
 * <p>
 * Connection pool configuration:
 * </p>
 * <ul>
 * <li>{@code maxPoolSize=n}: The maximum number of connections in the
 * connection pool.</li>
 * <li>{@code waitQueueMultiple=n} : this multiplier, multiplied with the
 * maxPoolSize setting, gives the maximum number of threads that may be waiting
 * for a connection to become available from the pool. All further threads will
 * get an exception right away.</li>
 * <li>{@code waitQueueTimeoutMS=ms}: The maximum wait time in milliseconds that
 * a thread may wait for a connection to become available.</li>
 * </ul>
 * <p>
 ...
 * <ul>
 * <li>{@code true}: the driver sends a getLastError command after every update
 * to ensure that the update succeeded (see also {@code w} and
 * {@code wtimeoutMS}).</li>
 * <li>{@code false}: the driver does not send a getLastError command after
 * every update.</li>
 * </ul>
 * </li>
 * <li>{@code w=wValue}
 * <ul>
 * <li>The driver adds { w : wValue } to the getLastError command. Implies
 * {@code safe=true}.</li>
 * <li>wValue is typically a number, but can be any string in order to allow for
 * specifications like {@code "majority"}</li>
 * </ul>
 * </li>
 * <li>{@code wtimeoutMS=ms}
 * <ul>
 * <li>The driver adds { wtimeout : ms } to the getlasterror command. Implies
 * {@code safe=true}.</li>
 ...
 * </p>
 * <ul>
 * <li>{@code slaveOk=true|false}: Whether a driver connected to a replica set
 * will send reads to slaves/secondaries.</li>
 * <li>{@code readPreference=enum}: The read preference for this connection. If
 * set, it overrides any slaveOk value.
 * <ul>
 ...
 * </ul>
 * </li>
 * <li>{@code readPreferenceTags=string}. A representation of a tag set as a
 * comma-separated list of colon-separated key-value pairs, e.g.
 * {@code "dc:ny,rack:1}". Spaces are stripped from beginning and end of all
 * keys and values. To specify a list of tag sets, using multiple
 * readPreferenceTags, e.g.
 * {@code readPreferenceTags=dc:ny,rack:1;readPreferenceTags=dc:ny;readPreferenceTags=}
 * <ul>
 * <li>Note the empty value for the last one, which means match any secondary as
 * a last resort.</li>
 ...
 * </p>
 * <ul>
 * <li>{@code authMechanism=MONGO-CR|GSSAPI|PLAIN|MONGODB-X509}: The
 * authentication mechanism to use if a credential was supplied. The default is
 * MONGODB-CR, which is the native MongoDB Challenge Response mechanism. For the
 * GSSAPI and MONGODB-X509 mechanisms, no password is accepted, only the
 * username.</li>
 * <li>{@code authSource=string}: The source of the authentication credentials.
 * This is typically the database that the credentials have been created. The
 * value defaults to the database specified in the path portion of the URI. If
 * the database is specified in neither place, the default value is "admin". For
 * GSSAPI, it's not necessary to specify a source.</li>
 * <li>{@code gssapiServiceName=string}: This option only applies to the GSSAPI
 * mechanism and is used to alter the service name..</li>
 * <ul>
 * <p>
 * Note: This class is a replacement for {@code MongoURI}, to be used with
 * {@code MongoClient}. The main difference in behavior is that the default
 * write concern is {@code WriteConcern.ACKNOWLEDGED}.
 * </p>
 * 
...
public class MongoClientURI {
    ...
    /**
     * Creates a MongoURI from the given URI string, and
     * MongoClientOptions.Builder. The builder can be configured with default
	...
	try {
		...
		if (idx > 0) {
			...
			userName = URLDecoder.decode(
				authPart.substring(0, idx), UTF_8);
			password = URLDecoder.decode(
				authPart.substring(idx + 1), UTF_8)
				.toCharArray();
		    }
		}
	...
	return in != null
		&& in.length() > 0
		&& (in.equals("1") || in.toLowerCase().equals("true") || in
			.toLowerCase().equals("yes"));
    }

    private ReadPreference buildReadPreference(final String readPreferenceType,
	    final DBObject firstTagSet, final List<DBObject> remainingTagSets,
	...
	if (readPreferenceType != null) {
		...
		return ReadPreference.valueOf(readPreferenceType, firstTagSet,
			remainingTagSets.toArray(new DBObject[remainingTagSets
				.size()]));
	    }
	...
	if (w != null || wTimeout != 0 || fsync || journal) {
		...
		try {
		    return new WriteConcern(Integer.parseInt(w), wTimeout,
			    fsync, journal);
    ...
    private MongoCredential createCredentials(
	    Map<String, List<String>> optionsMap, final String userName,
	...
	if (mechanism.equals(MongoCredential.GSSAPI_MECHANISM)) {
	    MongoCredential gssapiCredential = MongoCredential
		    .createGSSAPICredential(userName);
	    if (gssapiServiceName != null) {
		gssapiCredential = gssapiCredential.withMechanismProperty(
			"SERVICE_NAME", gssapiServiceName);
	    }
	    return gssapiCredential;
	} else if (mechanism.equals(MongoCredential.PLAIN_MECHANISM)) {
	    return MongoCredential.createPlainCredential(userName, authSource,
		    password);
	} else if (mechanism.equals(MongoCredential.MONGODB_CR_MECHANISM)) {
	    return MongoCredential.createMongoCRCredential(userName,
		    authSource, password);
	} else if (mechanism.equals(MongoCredential.MONGODB_X509_MECHANISM)) {
	    return MongoCredential.createMongoX509Credential(userName);
	} else {
	    throw new IllegalArgumentException("Unsupported authMechanism: "
		    + mechanism);
	}
    }
    ...
    private ReadPreference createReadPreference(
	...
	return buildReadPreference(readPreferenceType, firstTagSet,
		remainingTagSets, slaveOk);
    }
    ...
    private String getLastValue(final Map<String, List<String>> optionsMap,
	    final String key) {
	...
	if (tagSetString.length() > 0) {
		...
		if (tagKeyValuePair.length != 2) {
		    throw new IllegalArgumentException(
			    "Bad read preference tags: " + tagSetString);
		}
