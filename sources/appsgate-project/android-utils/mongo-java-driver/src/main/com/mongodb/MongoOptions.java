...
/**
 * Various settings for a Mongo instance. Not thread safe, and superseded by
 * MongoClientOptions. This class may be deprecated in a future release.
 * 
...
public class MongoOptions {
    ...
    /**
     * <p>
     * The description for <code>Mongo</code> instances created with these
     * options. This is used in various places like logging.
     * </p>
     */
     ...
     * pool is exhausted, any operation requiring a connection will block
     * waiting for an available connection. Default is 10.
     * 
     ...
     * connection to become available. Default is 120,000. A value of 0 means
     * that it will not wait. A negative value means to wait indefinitely.
     */
     ...
     * {@link java.net.Socket#connect(java.net.SocketAddress, int) } Default is
     * 10,000.
     */
     ...
     * write operations {@link java.net.Socket#setSoTimeout(int)} Default is 0
     ...
     * alive through firewalls {@link java.net.Socket#setKeepAlive(boolean)}
     * Default is false.
     */
     ...
     * case that the socket cannot be established. There is maximum amount of
     * time to keep retrying, which is 15s by default. This can be useful to
     * avoid some exceptions being thrown when a server is down temporarily by
     * blocking the operations. It also can be useful to smooth the transition
     * to a new master (so that a new master is elected within the retry time).
     * Note that when using this flag: - for a replica set, the driver will
     * trying to connect to the old master for that time, instead of failing
     * over to the new one right away - this does not prevent exception from
     * being thrown in read/write operations on the socket, which must be
    ...
    /**
     * This flag specifies if the driver is allowed to read from secondary
     * (slave) servers. Specifically in the current implementation, the driver
     * will avoid reading from the primary server and round robin requests to
     * secondaries. Driver also factors in the latency to secondaries when
     * choosing a server. Note that reading from secondaries can increase
     * performance and reliability, but it may result in temporary inconsistent
    ...
    /**
     * Override the encoding factory. Default is for the standard Mongo Java
     * driver configuration.
     */
    public DBEncoderFactory dbEncoderFactory;

    /**
     * If <b>true</b> the driver will use a WriteConcern of WriteConcern.SAFE
     * for all operations. If w, wtimeout, fsync or j are specified, this
     * setting is ignored. Default is false.
     */
    public boolean safe;

    /**
     * The "w" value, (number of writes), of the global WriteConcern. Default is
     * 0.
     */
    public int w;
    ...
    /**
     * The "fsync" value of the global WriteConcern. true indicates writes
     * should wait for data to be written to server data file Default is false.
     */
    public boolean fsync;

    /**
     * The "j" value of the global WriteConcern. true indicates writes should
     * wait for a journaling group commit Default is false.
     */
    public boolean j;

    /**
     * sets the socket factory for creating sockets to mongod Default is
     ...
     * instances of DBCursor that the client does not close. If you are careful
     * to always call the close method of DBCursor, then this can safely be set
     * to false.
     * 
    ...
    /**
     * Sets the write concern. If this is not set, the write concern defaults to
     * the combination of settings of the other write concern-related fields. If
     * set, this will override all of the other write concern-related fields.
     * 
     * @see #w
     * @see #safe
     * @see #wtimeout
	...
	slaveOk = false; // default to false, as readPreference field will be
			 // responsible
	...
	if (dbDecoderFactory != null ? !dbDecoderFactory
		.equals(options.dbDecoderFactory)
		: options.dbDecoderFactory != null) {
	    return false;
	}
	if (dbEncoderFactory != null ? !dbEncoderFactory
		.equals(options.dbEncoderFactory)
		: options.dbEncoderFactory != null) {
	    return false;
	}
	if (description != null ? !description.equals(options.description)
		: options.description != null) {
	    return false;
	}
	if (readPreference != null ? !readPreference
		.equals(options.readPreference)
		: options.readPreference != null) {
	    return false;
	}
	if (socketFactory != null ? !socketFactory
		.equals(options.socketFactory) : options.socketFactory != null) {
	    return false;
	}
	if (writeConcern != null ? !writeConcern.equals(options.writeConcern)
    ...
    /**
     * 
     * @return the maximum number of connections allowed per host for this Mongo
     *         instance
     */
    ...
    /**
     * @return The description for <code>MongoClient</code> instances created
     *         with these options
     */
    ...
    /**
     * 
     * @return The maximum time in milliseconds that threads wait for a
     *         connection
     */
    ...
    /**
     * 
     * @return the maximum number of threads that may be waiting for a
     *         connection
     */
    ...
    /**
     * Helper method to return the appropriate WriteConcern instance based on
     * the current related options settings.
     **/
	...
	result = 31
		* result
		+ (int) (maxAutoConnectRetryTime ^ (maxAutoConnectRetryTime >>> 32));
	result = 31 * result + (slaveOk ? 1 : 0);
	result = 31 * result
		+ (readPreference != null ? readPreference.hashCode() : 0);
	result = 31 * result
		+ (dbDecoderFactory != null ? dbDecoderFactory.hashCode() : 0);
	result = 31 * result
		+ (dbEncoderFactory != null ? dbEncoderFactory.hashCode() : 0);
	...
	result = 31 * result
		+ (writeConcern != null ? writeConcern.hashCode() : 0);
	return result;
    }
     ...
     *            sets whether the driver should always use MBeans, regardless
     *            of VM
     */
     ...
     * 
     * @param retry
     *            sets keep trying connection flag
     */
     ...
     * 
     * @param connections
     *            sets the maximum number of connections allowed per host for
     *            this Mongo instance
     */
     ...
     * 
     * @param desc
     *            The description for <code>Mongo</code> instances created with
     *            these options
     */
     ...
     * 
     * @param retryTimeMS
     *            set max time in MS to retrying open connection
     */
     ...
     * 
     * @param timeMS
     *            set the maximum time in milliseconds that threads wait for a
     *            connection
     */
     ...
     * 
     * @param threads
     *            multiplied with connectionsPerHost, sets the maximum number of
     *            threads that may be waiting for a connection
     */
    public synchronized void setThreadsAllowedToBlockForConnectionMultiplier(
	    int threads) {
	...
	return "MongoOptions{" + "description='" + description + '\''
		+ ", connectionsPerHost=" + connectionsPerHost
		+ ", threadsAllowedToBlockForConnectionMultiplier="
		+ threadsAllowedToBlockForConnectionMultiplier
		+ ", maxWaitTime=" + maxWaitTime + ", connectTimeout="
		...
		+ ", slaveOk=" + slaveOk + ", readPreference=" + readPreference
		+ ", dbDecoderFactory=" + dbDecoderFactory
		+ ", dbEncoderFactory=" + dbEncoderFactory + ", safe=" + safe
		+ ", w=" + w + ", wtimeout=" + wtimeout + ", fsync=" + fsync
		+ ", j=" + j + ", socketFactory=" + socketFactory
		+ ", cursorFinalizerEnabled=" + cursorFinalizerEnabled
		+ ", writeConcern=" + writeConcern + ", alwaysUseMBeans="
		+ alwaysUseMBeans + '}';
    }
}
