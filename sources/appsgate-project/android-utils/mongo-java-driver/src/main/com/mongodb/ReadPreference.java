...
/**
 * An abstract class that represents preferred replica set members to which a
 * query or command can be sent.
 * 
 * @mongodb.driver.manual applications/replication/#replica-set-read-preference
 *                        Read Preference
 */
public abstract class ReadPreference {
	...
	private static List<DBObject> splitMapIntoMultipleMaps(DBObject tags) {
	    List<DBObject> tagList = new ArrayList<DBObject>(tags.keySet()
		    .size());
	...
	public TaggedReadPreference(DBObject tags) {
		...
		throw new IllegalArgumentException(
			"tags can not be null or empty");
	    }
	    _tags = tags;
	    List<DBObject> maps = splitMapIntoMultipleMaps(_tags);
	    _pref = new TaggableReadPreference.SecondaryReadPreference(
		    maps.get(0), getRemainingMaps(maps));
	}
	...
	private DBObject[] getRemainingMaps(final List<DBObject> maps) {
	    ...
	    return maps.subList(1, maps.size() - 1).toArray(
		    new DBObject[maps.size() - 1]);
	}
    ...
    /**
     * A primary read preference. Equivalent to calling
     * {@code ReadPreference.primary()}.
     * 
     ...
     * {@code ReadPreference.secondaryPreferred}. This reference should really
     * have been called {@code ReadPreference.SECONDARY_PREFERRED}, but the
     * naming of it preceded the idea of distinguishing between secondary and
     * secondary-preferred, so for backwards compatibility, leaving the name as
     * is with the behavior as it was when it was created.
     * 
	...
	PRIMARY = _PRIMARY;
	SECONDARY = _SECONDARY_PREFERRED; // this is not a bug. See SECONDARY
					  // Javadoc.
    }
	...
	return new TaggableReadPreference.NearestReadPreference(firstTagSet,
		remainingTagSets);
    }
    ...
    /**
     * @return ReadPreference which reads primary if available, otherwise a
     *         secondary respective of tags.
     */
    ...
    public static TaggableReadPreference secondary(DBObject firstTagSet,
	    DBObject... remainingTagSets) {
	return new TaggableReadPreference.SecondaryReadPreference(firstTagSet,
		remainingTagSets);
    }

    /**
     * @return ReadPreference which reads secondary if available, otherwise from
     *         primary.
     */
    ...
    /**
     * @return ReadPreference which reads secondary if available respective of
     *         tags, otherwise from primary irrespective of tags.
     */
    public static TaggableReadPreference secondaryPreferred(
	...
	throw new IllegalArgumentException("No match for read preference of "
		+ name);
    }

    public static TaggableReadPreference valueOf(String name,
	    DBObject firstTagSet, final DBObject... remainingTagSets) {
	if (name == null) {
	    throw new IllegalArgumentException();
	}
    ...
    /**
     * @return <code>true</code> if this preference allows reads or commands
