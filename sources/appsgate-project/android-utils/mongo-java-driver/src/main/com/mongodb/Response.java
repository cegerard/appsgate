...
class Response {
    ...
    Response(ServerAddress addr, DBCollection collection, InputStream in,
	...
	for (int i = 0; i < _num; i++) {
	    if (user._toGo < 5) {
		throw new IOException("should have more objects, but only "
			+ user._toGo + " bytes left");
		// TODO: By moving to generics, you can remove these casts (and
		// requirement to impl DBOBject).
	    }
	...
	if (user._toGo != 0) {
	    throw new IOException("finished reading objects but still have: "
		    + user._toGo + " bytes to read!' ");
	}
	...
	return "flags:" + _flags + " _cursor:" + _cursor + " _startingFrom:"
		+ _startingFrom + " _num:" + _num;
    }
}
