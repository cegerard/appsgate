...
public class CLI {
	...
	for (int i = 0; i < args.length; i++) {
		...
		System.out.println("length: " + read + " md5: "
			+ Util.toHex(digest));
		return;
	    }
	...
	System.out
		.println("      list                      : lists all files in the store");
	System.out
		.println("      put filename              : puts the file filename into the store");
	System.out
		.println("      get filename1 filename2   : gets filename1 from store and sends to filename2");
	System.out
		.println("      md5 filename              : does an md5 hash on a file in the db (for testing)");
    }
