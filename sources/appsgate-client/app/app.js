define(function(require, exports, module) {
  ...
  app.initialize = function() {
    ...
    var serverAddr = document.URL.replace(/http:\/\//i, "").replace(
      ...
      $("#settings-modal #valid-button").bind("click",
      onValidSettingsButton);
      ...
      $("#lost-connection-modal #change-addr-server").bind("click",
      onChangeAddrServerButton);
      $("#lost-connection-modal #reconnect-button").bind("click",
      onReconnectButton);
      ...
      dispatcher.on("WebSocketClose", function() {
        ...
        $("#lost-connection-modal .text-danger").html(
          "La connexion a &eacute;t&eacute; interrompue.");
          ...
          function onValidSettingsButton(e) {
            if ((e.type === "keyup" && e.keyCode === 13 || e.type === "click") &&
              $("#settings-modal .addr-server") !== "") {
                ...
                ":";
                serverAddr += $("#settings-modal .port-server").val() === "" ?
                "8080" : $("#settings-modal .port-server").val();

                // update the language if updated
                if ($("#settings-modal select#language :selected").val() !==
                  appRouter.locale) {
                    appRouter.updateLocale($(
                      "#settings-modal select#language :selected").val());
                      $("#settings-modal").modal("hide");
                    }
