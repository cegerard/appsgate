...
define([
  ...
  ProgramKeyboardBuilder = Backbone.Model.extend({
    ...
    buildKeyboard: function(ex) {
      //console.debug("Build the Keyboard !!! with ex : ", ex);
    ...
    buildPrograms: function() {
      ...
      $(btnCall).append("<span>" + $.i18n.t('programs.keyboard.actionActivate', {
        myVar: "<span class='highlight-placeholder'>" + $.i18n.t('programs.keyboard.program') + "</span>",
      }));
      var v = {
        "type": "action",
      ...
      $(btnStop).append("<span>" + $.i18n.t('programs.keyboard.actionDisactivate', {
        myVar: "<span class='highlight-placeholder'>" + $.i18n.t('programs.keyboard.program') + "</span>",
      }));
      var w = {
        "type": "action",
    ...
    buildStateProgramKeys: function(which) {
      ...
      $(btn).append("<span>" + $.i18n.t('programs.keyboard.stateStarted' + keep, {
        myVar: "<span class='highlight-placeholder'>" + $.i18n.t('programs.keyboard.program') + "</span>",
      }));
      ...
      $(btn2).append("<span>" + $.i18n.t('programs.keyboard.stateStopped' + keep, {
        myVar: "<span class='highlight-placeholder'>" + $.i18n.t('programs.keyboard.program') + "</span>",
      }));
    ...
    buildEventProgramKeys: function() {
      var btn = jQuery.parseHTML("<button class='btn btn-default btn-keyboard specific-node' group-id='program'></button>");
      $(btn).append("<span>" + $.i18n.t('programs.keyboard.eventStart', {
        myVar: "<span class='highlight-placeholder'>" + $.i18n.t('programs.keyboard.program') + "</span>",
      }));
      var v = {
        "type": "eventProgram",
      ...
      var btn2 = jQuery.parseHTML("<button class='btn btn-default btn-keyboard specific-node' group-id='program'></button>");
      $(btn2).append("<span>" + $.i18n.t('programs.keyboard.eventStop', {
        myVar: "<span class='highlight-placeholder'>" + $.i18n.t('programs.keyboard.program') + "</span>",
      }));
      var w = {
        "type": "eventProgram",
    ...
    sortKeyband: function(keyband) {
      var keyband = $(keyband);
      var deviceTypes = devices.groupBy(function(device) {
        return device.get("type")
      });
      var serviceTypes = services.groupBy(function(service) {
      ...
      if (keyband.children().length < 1) {
        ...
        if (keyband.hasClass("expected-actions") || keyband.hasClass("expected-events") || keyband.hasClass("expected-links")) {
          $(groups).append("<div class='row devices'/><div class='row services'/><div class='row programs'/>");
        }
        ...
        $.each(buttons, function(idx, itm) {
          ...
          if (groups.querySelectorAll('#group-' + $(itm).attr('group-id')).length === 0) {
            var groupTemplate = "<span id='group-" + $(itm).attr('group-id') + "'></span>&nbsp;";
            if (keyband.hasClass("expected-actions") || keyband.hasClass("expected-events") || keyband.hasClass("expected-links")) {
              if ($(itm).attr('group-id') == "program") {
                $(groups).children(".programs").append(groupTemplate);
              } else if (deviceTypes.hasOwnProperty($(itm).attr('group-id'))) {
                $(groups).children(".devices").append(groupTemplate);
              } else if (serviceTypes.hasOwnProperty($(itm).attr('group-id'))) {
                $(groups).children(".services").append(groupTemplate);
              } else {
          ...
          $(groups.querySelectorAll('#group-' + $(itm).attr('group-id'))).append(itm);
        })
