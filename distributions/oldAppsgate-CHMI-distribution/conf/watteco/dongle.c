...
serial_to_tun(FILE *inslip, int outfd)
{
  ...
  if(ret == -1 || ret == 0) printf("serial_to_tun(1): read\n");//err(1, "serial_to_tun: read");
  goto after_fread;
#endif
...
/*	
		...
		if ((strncmp("PanID@",&(uip.inbuf[1]),6) == 0) && ( strlen(panidfile)>0)) {
	        FILE * fdmpo;
...
*/  
	...
	// Process a returned SLIP configuration message containing addresses
//	if ((strncmp("IPv6@",&(uip.inbuf[1]),5) == 0) && ( strlen(ipv6addrfile)>0)) {
	if (strncmp("IPv6@",&(uip.inbuf[1]),5) == 0) {
/*
        FILE * fdpeg;
...
*/
	int i;
	char *pt=&(uip.inbuf[1]);
	for (i=0;i<inbufptr-1;i++) {
	  if (pt[i]=='f') {
	    if ((pt[i+1]=='e')&&(pt[i+2]=='8')&&(pt[i+3]=='0')) {
	      int j;				
		for (j=0;j<18;j++) {				
		  fprintf(stdout, "%c", pt[i+j]);
		}
		fprintf(stdout, "\n");
		exit(0);
	      }
...
//    if(siodev != NULL) {
      slipfd = devopen(siodev, O_RDWR | O_NONBLOCK | O_EXCL);
      if(slipfd == -1) {
